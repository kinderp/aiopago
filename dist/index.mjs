  ...
  "src/plan-store.mjs"() {
    ...
    PlanRevisionWriter = class {
      ...
      constructor(path = "TASK_PLAN.md", options = {}) {
        ...
        this.guardianRoot = resolve2(options.guardianRoot ?? join2(dirname2(this.path), ".guardian"));
        this.lockPath = resolve2(options.lockPath ?? join2(this.guardianRoot, "plan-write.lock"));
...
`, "utf8");
      ...
      #redurabilizeExisting(path, identity2) {
        ...
        try {
          ...
          this.#io.closeSync(fd);
          fd = void 0;
          syncDirectory(this.#io, dirname2(path));
        } finally {
          closeQuietly(this.#io, fd);
        }
      }
      writeImmutable(path, bytes, { conflictCode = "PLAN_PROVENANCE_CONFLICT", maximum = MAX_PLAN_STATE_BYTES, allowExistingExact = true } = {}) {
        ...
        const parent = dirname2(destination);
      ...
      #prepareCandidateTemp(bytes, mode, baseIdentity) {
        ...
        try {
          ...
          this.#io.closeSync(fd);
          fd = void 0;
          return { path: temp, identity: identity2, reference: relative(dirname2(this.path), temp).replaceAll("\\", "/"), ownsTemp };
        } catch (error) {
          closeQuietly(this.#io, fd);
          if (ownsTemp) {
            try {
      ...
      #persistHistory(current) {
        ...
        const path = resolve2(dirname2(this.path), reference);
        try {
      ...
      commit({
        ...
        try {
          ...
          syncDirectory(this.#io, dirname2(this.path));
...
function canonicalUtc(value, field) {
  ...
  const iso2 = new Date(milliseconds).toISOString();
  invariant(value === iso2 || value === iso2.replace(".000Z", "Z"), "LEDGER_TIMESTAMP_INVALID", `${field} must be canonical RFC 3339 UTC`);
  return value;
}
...
async function resolvePiRoot(options = {}) {
  ...
  if (configuredRoot) {
    const root = resolve4(configuredRoot);
    try {
...
async function inspectPi(options = {}) {
  ...
  let manifest;
  try {
    manifest = JSON.parse(await readFile(join3(root, "package.json"), "utf8"));
  } catch (error) {
...
function validateRepositoryStateBoundaries(targetRoot) {
  ...
  for (const [localPath, expectedType] of reserved) inspectReservedPath(join4(targetRoot, localPath), expectedType);
}
function discoverTargetRepository(input = process.cwd(), options = {}) {
  const startPath = realDirectory(resolve5(options.baseDirectory ?? process.cwd(), input));
...
function resolveInside(targetRoot, configuredPath, field) {
  ...
  const absolute = resolve5(targetRoot, configuredPath);
...
function validateRepositoryConfig(config, targetRoot) {
  ...
  const configRoot = join4(targetRoot, ".guardian");
...
import { dirname as dirname5, join as join6, relative as relative3, resolve as resolve6 } from "node:path";
  ...
  "src/plan-proposal.mjs"() {
    ...
    PlanPort = class {
      ...
      constructor(path = "TASK_PLAN.md", options = {}) {
        ...
        this.provenanceRoot = resolve6(options.provenanceRoot ?? join6(dirname5(this.path), ".guardian", "plan-proposals"));
      }
      proposal(input) {
      ...
      apply(input) {
        ...
        const recordRoot = join6(this.provenanceRoot, recordId);
        const registrationPath = join6(recordRoot, "proposal.json");
        const attemptsRoot = join6(recordRoot, "attempts");
        const appliedPath = join6(recordRoot, "applied.json");
        const provenanceReference = relative3(dirname5(this.path), appliedPath).replaceAll("\\", "/");
        ...
        const inspectTree = () => {
          ...
          if (this.writer.stateExists(attemptsRoot)) {
            ...
            for (const entry of attemptEntries) {
              ...
              const path = join6(attemptsRoot, entry.name);
              ...
              intents.push({ path, reference: relative3(dirname5(this.path), path).replaceAll("\\", "/"), value });
            }
          }
        ...
        return deepFreeze(this.writer.commit({
          ...
          inspectExisting: (current) => {
            ...
            const snapshotPath = resolve6(dirname5(this.path), previousSnapshotReference);
          ...
          prepare: (current, { previousSnapshotReference }) => {
            ...
            return {
              ...
              beforeFinalAttestation: ({ candidateTempIdentity, candidateTempReference }) => {
                ...
                activeIntentPath = join6(attemptsRoot, `${attemptToken}.json`);
                activeIntentReference = relative3(dirname5(this.path), activeIntentPath).replaceAll("\\", "/");
...
function observeHumanWorkflow(input = process.cwd(), options = {}) {
  ...
  const runtime = plan2.valid && options.includeRuntime !== false ? (options.readRuntimeProjection ?? readRuntimeProjection)(join7(repository.runtimeRoot, "guardian.sqlite"), plan2.plan, options.runtimeOptions) : EMPTY_RUNTIME;
...
function boundedString2(value, code, { nullable = false, max = 512 } = {}) {
...
function emptyCalibrationQualityEvidence(runId) {
  boundedString2(runId, "QUALITY_RUN_ID_INVALID", { max: 128 });
  return {
...
function validateCalibrationQualityEvidence(evidence, { runId = evidence?.run_id, expectedCommands = [] } = {}) {
  exactKeys2(evidence, ["schema_version", "run_id", "accepted_checkpoints", "completion_marker_observed", "controls", "gate_attempts", "review_findings", "rework_cycles", "regressions", "final_acceptance"], "QUALITY_EVIDENCE_FIELDS_INVALID");
  ...
  for (const id of evidence.accepted_checkpoints) boundedString2(id, "QUALITY_CHECKPOINT_ID_INVALID", { max: 128 });
  unique(evidence.accepted_checkpoints, "QUALITY_CHECKPOINT_DUPLICATE");
  boundedString2(evidence.completion_marker_observed, "QUALITY_COMPLETION_MARKER_INVALID", { nullable: true });
  exactKeys2(evidence.controls, ["no_conversation_history_saved", "cold_review_new_session", "operator_protocol_deviation", "external_services_used"], "QUALITY_CONTROLS_INVALID");
  ...
  for (const gate of evidence.gate_attempts) {
    exactKeys2(gate, ["suite_attempt", "ordinal", "command", "started_at", "ended_at", "exit_code", "output_sha256"], "QUALITY_GATE_FIELDS_INVALID");
  ...
  for (const finding of evidence.review_findings) {
    exactKeys2(finding, ["finding_id", "severity", "status", "code", "artifact_path", "artifact_sha256"], "QUALITY_REVIEW_FINDING_FIELDS_INVALID");
    boundedString2(finding.finding_id, "QUALITY_FINDING_ID_INVALID", { max: 128 });
    ...
    boundedString2(finding.code, "QUALITY_FINDING_CODE_INVALID", { max: 128 });
    boundedString2(finding.artifact_path, "QUALITY_FINDING_ARTIFACT_INVALID", { nullable: true, max: 512 });
  ...
  for (const cycle of evidence.rework_cycles) {
    exactKeys2(cycle, ["cycle_id", "status", "trigger_refs", "files_changed"], "QUALITY_REWORK_FIELDS_INVALID");
    boundedString2(cycle.cycle_id, "QUALITY_REWORK_ID_INVALID", { max: 128 });
    ...
    for (const value of [...cycle.trigger_refs, ...cycle.files_changed]) boundedString2(value, "QUALITY_REWORK_VALUE_INVALID", { max: 512 });
  ...
  for (const regression of evidence.regressions) {
    exactKeys2(regression, ["regression_id", "status", "code"], "QUALITY_REGRESSION_FIELDS_INVALID");
    boundedString2(regression.regression_id, "QUALITY_REGRESSION_ID_INVALID", { max: 128 });
    boundedString2(regression.code, "QUALITY_REGRESSION_CODE_INVALID", { max: 128 });
  ...
  if (evidence.final_acceptance !== null) {
    exactKeys2(evidence.final_acceptance, ["decision_id", "status", "decided_at"], "QUALITY_FINAL_ACCEPTANCE_FIELDS_INVALID");
    boundedString2(evidence.final_acceptance.decision_id, "QUALITY_FINAL_ACCEPTANCE_ID_INVALID", { max: 128 });
...
function writeCalibrationQualityEvidence(path, evidence, options = {}) {
  const resolved = resolve8(path);
...
`;
  ...
  let fd;
  try {
    fd = openSync3(temp, "w", 384);
    writeFileSync4(fd, bytes, "utf8");
    fsyncSync3(fd);
    closeSync3(fd);
    fd = void 0;
    renameSync3(temp, resolved);
    try {
      const dirFd = openSync3(dirname6(resolved), "r");
      fsyncSync3(dirFd);
      closeSync3(dirFd);
    } catch {
    }
...
var CALIBRATION_QUALITY_SCHEMA, LEGACY_CALIBRATION_QUALITY_SCHEMA, QUALITY_EVIDENCE_FILE, SHA256, ISO;
var init_calibration_quality = __esm({
  "src/calibration-quality.mjs"() {
    init_canonical();
    init_errors();
    CALIBRATION_QUALITY_SCHEMA = "aiopago.calibration-quality-evidence/1.0.0";
    LEGACY_CALIBRATION_QUALITY_SCHEMA = "eiopago.calibration-quality-evidence/1.0.0";
    QUALITY_EVIDENCE_FILE = "quality-evidence.json";
...
import { dirname as dirname7, isAbsolute as isAbsolute2, join as join8, relative as relative4, resolve as resolve9 } from "node:path";
...
function writeJson(path, value) {
  writeFileSync5(path, `${canonicalJson(value)}
...
function makeCalibrationRunId(variantId) {
  ...
  return `H2-02B-${variantId}-${randomUUID3()}`;
}
function calibrationPaths(cwd, runId, calibrationRoot = join8(cwd, ".guardian", "calibration")) {
  invariant(safeRunId(runId), "CALIBRATION_RUN_ID_INVALID", runId);
  const runRoot = resolve9(calibrationRoot, runId);
  return Object.freeze({
    calibrationRoot: resolve9(calibrationRoot),
    runRoot,
    protocolCopyPath: join8(runRoot, "pilot-protocol.json"),
    workloadProtocolPath: join8(resolve9(calibrationRoot), "pilot-protocol.json"),
    attestationPath: join8(runRoot, "preflight-attestation.json"),
    runtimeRoot: join8(runRoot, "runtime"),
    runtimeStorePath: join8(runRoot, "runtime", "guardian.sqlite"),
    qualityEvidencePath: join8(runRoot, QUALITY_EVIDENCE_FILE),
    runRecordPath: join8(runRoot, "run-record.json")
  });
}
...
function runCalibrationPreflight({
  cwd = process.cwd(),
  variantId,
  experimentBaselineCommit,
  protocolPath = join8(cwd, DEFAULT_CALIBRATION_PROTOCOL),
  calibrationRoot = join8(cwd, ".guardian", "calibration"),
  runId = makeCalibrationRunId(variantId),
  piRoot,
  now = utcNow()
} = {}) {
  cwd = resolve9(cwd);
  protocolPath = resolve9(protocolPath);
  invariant(safeRunId(runId), "CALIBRATION_RUN_ID_INVALID", runId);
  invariant(/^[a-f0-9]{40}$/.test(experimentBaselineCommit ?? ""), "EXPERIMENT_BASELINE_INVALID");
  invariant(piRoot && isAbsolute2(piRoot), "PI_ROOT_REQUIRED");
  ...
  try {
    ...
    failures.push(reason("HEAD_UNAVAILABLE"));
  }
  ...
  try {
    ...
    failures.push(reason("BRANCH_UNAVAILABLE"));
  }
  try {
    status = git(cwd, ["status", "--porcelain=v1", "--untracked-files=all"]);
  } catch {
    failures.push(reason("GIT_STATUS_UNAVAILABLE"));
  }
  ...
  if (protocolBytes && head === experimentBaselineCommit) {
    ...
    if (protocolRelative.startsWith("../") || protocolRelative === "..") failures.push(reason("PROTOCOL_OUTSIDE_WORKTREE"));
      ...
      try {
        frozenProtocolBytes = git(cwd, ["show", `${experimentBaselineCommit}:${protocolRelative}`], "buffer");
      } catch {
        failures.push(reason("FROZEN_PROTOCOL_MISSING"));
      }
  ...
  let piVersion = null;
  try {
    piVersion = readPiVersion(piRoot);
  } catch {
    failures.push(reason("PI_VERSION_UNAVAILABLE"));
  }
  ...
  try {
    effectiveThreshold = contextHandoffThreshold(requestedThreshold);
  } catch {
    failures.push(reason("THRESHOLD_INVALID"));
  }
  ...
  try {
    mkdirSync4(paths.runRoot, { recursive: false });
  } catch (error) {
    if (error.code === "ENOENT") {
      mkdirSync4(dirname7(paths.runRoot), { recursive: true });
      try {
        mkdirSync4(paths.runRoot, { recursive: false });
      } catch (again) {
        throw new GuardianError(again.code === "EEXIST" ? "DUPLICATE_RUN_ID" : "CALIBRATION_RUN_ROOT_CREATE_FAILED", again.message);
      }
    } else throw new GuardianError(error.code === "EEXIST" ? "DUPLICATE_RUN_ID" : "CALIBRATION_RUN_ROOT_CREATE_FAILED", error.message);
  }
  if (protocolBytes) {
    writeFileSync5(paths.protocolCopyPath, protocolBytes, { flag: "wx" });
...
function loadCalibrationAttestation(attestationPath) {
  ...
  return { attestation, bytes, digest: digestHex(bytes), path: resolve9(attestationPath) };
}
function calibrationRunnerOptions(attestationPath, { resume = false } = {}) {
...
function markCalibrationRunInvalid(runRecordPath, error) {
  ...
  writeFileSync5(runRecordPath, `${canonicalJson(record)}
`, "utf8");
  return record;
}
function assertFreshRuntimeStore(path) {
  try {
    statSync2(path);
    throw new GuardianError("STALE_RUNTIME_STORE", path);
  } catch (error) {
    if (error.code !== "ENOENT") throw error;
  }
}
var CALIBRATION_ATTESTATION_SCHEMA, LEGACY_CALIBRATION_ATTESTATION_SCHEMA, CALIBRATION_RUN_RECORD_SCHEMA, DEFAULT_CALIBRATION_PROTOCOL;
var init_calibration_preflight = __esm({
  "src/calibration-preflight.mjs"() {
    ...
    CALIBRATION_RUN_RECORD_SCHEMA = "aiopago.calibration-run-record/1.0.0";
    DEFAULT_CALIBRATION_PROTOCOL = "docs/m1-h2-calibration-pilot.json";
  }
});
...
function formatGuardianStatus(runner, ctx = null) {
...
function worktreeDigest(workdir) {
  ...
  for (const path of paths) {
    const absolute = resolve10(workdir, path);
    let kind = "missing";
    let mode = 0;
...
function observeGitState(cwd = process.cwd()) {
  const workdir = realpathSync3(resolve10(cwd)).replaceAll("\\", "/");
  ...
  "src/handoff.mjs"() {
    ...
    HandoffService = class {
      ...
      continuity(handoffId, targetSession) {
        ...
        verifyRequiredLocalPaths(dirname8(this.ledger.path), m.required_local_paths);
      ...
      #captureResumeAuthority(handoffId, targetSession, coordinatedPlan, expectedAuthority = null) {
        ...
        verifyRequiredLocalPaths(dirname8(this.ledger.path), manifest.payload.required_local_paths);
        invariant(
  ...
  "src/safety.mjs"() {
    ...
    AdmissionGate = class {
      ...
      streamDone() {
        ...
        if (this.activeStreams === 0) for (const resolve15 of this.waiters) resolve15();
  ...
  "src/storage.mjs"() {
    ...
    GuardianStorage = class {
      constructor(path = ".guardian/runtime/guardian.sqlite") {
        this.path = resolve12(path);
        mkdirSync5(dirname9(this.path), { recursive: true });
        const { DatabaseSync: DatabaseSync2 } = require2("node:sqlite");
        const connection = new DatabaseSync2(this.path);
  ...
  "src/runner.mjs"() {
    ...
    GuardianRunner = class _GuardianRunner {
      static async create(options = {}) {
        ...
        const cwd = resolve13(requestedRoot);
        const pi = options.pi ?? await loadPi({ searchRoot: cwd });
        const ledger = options.ledger ?? new TaskLedger(options.ledgerPath ?? repository?.taskLedgerPath ?? join9(cwd, "TASK_PLAN.md"));
        const plan2 = ledger.read();
        const storage = options.storage ?? new GuardianStorage(options.storagePath ?? join9(repository?.runtimeRoot ?? join9(cwd, ".guardian", "runtime"), "guardian.sqlite"));
        ...
        const artifacts = options.artifacts ?? new ArtifactStore(options.artifactRoot ?? repository?.artifactRoot ?? join9(cwd, ".guardian"), storage);
        ...
        const model = options.model ?? (policyProvider && policyModel ? modelRuntime.getModel(policyProvider, policyModel) : void 0);
...
var GITIGNORE_START = "# Aiopago local state (managed by aio init)";
var GITIGNORE_END = "# End Aiopago local state";
var LEGACY_GITIGNORE_START = "# Eiopago local state (managed by eio init)";
var LEGACY_GITIGNORE_END = "# End Eiopago local state";
var MANAGED_GITIGNORE_BODY = [
  "!.guardian/",
  ".guardian/*",
  ".guardian/runtime/",
...
var GITIGNORE_LINES = [GITIGNORE_START, ...MANAGED_GITIGNORE_BODY, GITIGNORE_END];
var LEGACY_GITIGNORE_LINES = [LEGACY_GITIGNORE_START, ...MANAGED_GITIGNORE_BODY, LEGACY_GITIGNORE_END];
...
function ensureGitignore(targetRoot, actions) {
  ...
  if (existed) {
    ...
    writeFileSync3(path, block, { encoding: "utf8", flag: "wx" });
...
async function initializeRepository(input = process.cwd(), options = {}) {
  ...
  const guardianRoot = join5(targetRoot, ".guardian");
  const configPath = join5(targetRoot, REPOSITORY_CONFIG_FILE);
  const ledgerPath = join5(targetRoot, "TASK_PLAN.md");
...
init_errors();
init_repository();
var AIO_VERSION = "0.1.0";
var HELP = `Aiopago portable alpha
...
var COMMANDS = /* @__PURE__ */ new Set(["init", "status", "why", "next", "plan", "start"]);
var READ_ONLY_COMMANDS = /* @__PURE__ */ new Set(["status", "why", "next", "plan"]);
var PLAN_OPTIONS = /* @__PURE__ */ new Set(["--raw", "--check", "--technical"]);
function parse(argv) {
...
var REQUIRED_HANDOFF_STATES = ["SUGGESTED", "PREPARED", "STARTED", "COMPLETED"];
var REQUIRED_ACCEPTANCE_COMMANDS = [
  "npm run check",
  "node --test --test-concurrency=1 test/handoff-inspector.test.mjs",
  "npm test",
  "git diff --check"
];
var PROTOCOL_SCHEMAS = /* @__PURE__ */ new Set([
  "aiopago.threshold-calibration-protocol/1.0.0",
  "eiopago.threshold-calibration-protocol/1.0.0"
]);
var BLOCKING_DIAGNOSTIC_STATUSES = /* @__PURE__ */ new Set(["collection_failed_no_metric_substitution", "measurement_missing"]);
function digestHex2(bytes) {
  return sha256(bytes).slice("sha256:".length);
}
...
function sumKnown(values) {
  return values.length > 0 && values.every(numeric) ? values.reduce((sum, value) => sum + value, 0) : null;
}
...
function unknownCost2(semantic) {
  return { amount: null, currency: null, status: "unknown", semantic };
}
function aggregateCost(samples, field, semantic, sampleSemantic, telemetryComplete) {
  ...
  const semanticallyValid = costs.length > 0 && costs.every((cost) => cost?.status === "available" && cost.currency === "USD" && cost.semantic === sampleSemantic && numeric(cost.amount));
  if (!semanticallyValid) return unknownCost2(semantic);
  return {
...
function artifactDigestStatus(worktree, finding) {
  ...
  if (finding.artifact_path === null || finding.artifact_sha256 === null || isAbsolute4(finding.artifact_path)) return "invalid";
...
function latestGateSuite(attempts, commands) {
  ...
  const complete = records.length === commands.length && records.every((item, index) => item.ordinal === index + 1 && item.command === commands[index]);
...
function nearestSuggestedSample(samples, event) {
  ...
  const candidates = samples.filter((sample) => sample.session_id === event.session_id && Number.isFinite(Date.parse(sample.captured_at)) && Date.parse(sample.captured_at) <= timestamp);
...
function latestEventBefore(events, state, sessionId, before) {
  ...
  return events.filter((event) => event.lifecycle_state === state && event.session_id === sessionId && Number.isFinite(Date.parse(event.timestamp)) && Date.parse(event.timestamp) <= limit).sort((a, b) => Date.parse(b.timestamp) - Date.parse(a.timestamp))[0] ?? null;
}
function handoffLifecycleChains(events, samples, threshold) {
  const ids = unique2(events.filter((event) => ["STARTED", "COMPLETED"].includes(event.lifecycle_state) && event.handoff_id).map((event) => event.handoff_id));
  const chains = ids.map((handoffId) => {
    const started = events.filter((event) => event.handoff_id === handoffId && event.lifecycle_state === "STARTED").sort((a, b) => Date.parse(a.timestamp) - Date.parse(b.timestamp))[0] ?? null;
    const completed = events.filter((event) => event.handoff_id === handoffId && event.lifecycle_state === "COMPLETED" && started && Date.parse(event.timestamp) >= Date.parse(started.timestamp)).sort((a, b) => Date.parse(a.timestamp) - Date.parse(b.timestamp))[0] ?? null;
  ...
  const complete = chains.length > 0 && noReusedSuggestion && chains.every((chain) => chain.suggested && chain.prepared && chain.started && chain.completed && chain.crossing);
  return { chains, complete };
}
function aggregateArtifacts(events) {
  const completedIds = unique2(events.filter((event) => event.lifecycle_state === "COMPLETED" && event.handoff_id).map((event) => event.handoff_id));
  const selected = [];
...
`;
  ...
  try {
    ...
    try {
      const dirFd = openSync4(dirname10(path), "r");
      fsyncSync4(dirFd);
      closeSync4(dirFd);
    } catch {
    }
...
function finalizeCalibrationRun({ runRoot, runId = null } = {}) {
  runRoot = resolve14(runRoot);
  const attestationPath = join10(runRoot, "preflight-attestation.json");
  const protocolPath = join10(runRoot, "pilot-protocol.json");
  const qualityPath = join10(runRoot, QUALITY_EVIDENCE_FILE);
  const outputPath = join10(runRoot, "run-record.json");
  const loaded = loadCalibrationAttestation(attestationPath);
  const { attestation } = loaded;
  ...
  if (digestHex2(protocolBytes) !== attestation.protocol_digest) sourceFailures.push("PROTOCOL_DIGEST_MISMATCH");
  ...
  try {
    ledger = new TaskLedger(join10(attestation.worktree, "TASK_PLAN.md")).read();
  } catch (error) {
    ledgerFailure = error.code ?? "WORKLOAD_LEDGER_INVALID";
  }
  let qualityEvidence = null;
  let qualityFailure = null;
  const acceptanceCommands = REQUIRED_ACCEPTANCE_COMMANDS;
  try {
    qualityEvidence = loadCalibrationQualityEvidence(qualityPath, { runId: attestation.run_id, expectedCommands: acceptanceCommands });
  } catch (error) {
    qualityFailure = error.code ?? "QUALITY_EVIDENCE_INVALID";
  }
  ...
  const perSessionCallCountsComplete = runtime.sessions.every((session) => {
    ...
    const calls = runtime.samples.filter((sample) => sample.session_id === session.session_id).map((sample) => sample.call_index).sort((a, b) => a - b);
  ...
  const coreTelemetryComplete = runtime.samples.length > 0 && usageComplete && contextComplete && callCountComplete && blockingDiagnostics.length === 0 && !sessionMeasurementMissing;
  ...
  const handoffCount = unique2(completedEvents.map((event) => event.handoff_id).filter(Boolean)).length;
  ...
  const handoff = {
    ...
    cost_overhead: unknownCost2("resume_calls_include_useful_work")
  };
  ...
  const suiteChronological = latestSuite.complete && latestSuite.records.every((gate, index) => index === 0 || Date.parse(gate.started_at) >= Date.parse(latestSuite.records[index - 1].ended_at));
  const decisionAfterSuite = latestSuite.complete && qualityEvidence?.final_acceptance && Date.parse(qualityEvidence.final_acceptance.decided_at) >= Date.parse(latestSuite.records.at(-1).ended_at);
  ...
  const reworkTriggersPresent = (qualityEvidence?.gate_attempts ?? []).some((item) => item.exit_code !== 0) || reviewFindings.some((item) => item.severity === "BLOCKING" && item.status === "RESOLVED") || (qualityEvidence?.regressions ?? []).some((item) => item.status === "RESOLVED");
  const reworkEvidenceComplete = !reworkTriggersPresent || (qualityEvidence?.rework_cycles ?? []).length > 0;
  const controlsPass = qualityEvidence?.controls?.no_conversation_history_saved === true && qualityEvidence?.controls?.cold_review_new_session === true && qualityEvidence?.controls?.operator_protocol_deviation === false && qualityEvidence?.controls?.external_services_used === false;
  const qualityPass = Boolean(qualityEvidence && latestSuite.pass && suiteChronological && decisionAfterSuite && taskComplete && acceptedCheckpoints.length === expectedCheckpoints.length && qualityEvidence.completion_marker_observed === protocol.workload?.completion_marker && !blockingReviewOpen && reviewArtifactsValid && unresolvedRegressions.length === 0 && openRework.length === 0 && reworkEvidenceComplete && controlsPass && qualityEvidence.final_acceptance?.status === "PASS");
  const quality = {
  ...
  const reasons = unique2(classificationReasons);
  let classification;
  let status;
  if (reasons.length) {
    classification = "INVALID";
    status = reasons.includes("INVALID_TELEMETRY") ? "INVALID_TELEMETRY" : reasons.some((item) => item.includes("PROTOCOL") || item.includes("CONTROLLED") || item.includes("PREFLIGHT") || item.includes("IDENTITY") || item.includes("RUNTIME")) ? "INVALID_CONTROLLED_VARIABLE" : reasons.includes("INVALID_HANDOFF_LIFECYCLE") ? "INVALID_HANDOFF" : "INVALID_QUALITY";
  ...
  const costPerCheckpoint = {
    charged: chargedCost.status === "available" && acceptedCount > 0 ? { amount: chargedCost.amount / acceptedCount, currency: "USD", status: "available", semantic: chargedCost.semantic } : unknownCost2(acceptedCount > 0 ? chargedCost.semantic : "no_accepted_checkpoint"),
    equivalent: equivalentCost.status === "available" && acceptedCount > 0 ? { amount: equivalentCost.amount / acceptedCount, currency: "USD", status: "available", semantic: equivalentCost.semantic } : unknownCost2(acceptedCount > 0 ? equivalentCost.semantic : "no_accepted_checkpoint")
  };
  const correlationPartialSamples = runtime.samples.filter((sample) => (sample.task_id ?? null) === null || sample.collection_status === "measurement_complete_correlation_partial");
  const telemetry = {
    ...
    measurement_status: coreTelemetryComplete ? correlationDiagnostics.length || correlationPartialSamples.length ? "measurement_complete_correlation_partial" : "measurement_complete" : "measurement_missing"
  };
...
export {
  ...
  CALIBRATION_FINALIZER_SCHEMA,
  CALIBRATION_QUALITY_SCHEMA,
  CALIBRATION_RUN_RECORD_SCHEMA,
  ...
  createGuardianExtension,
  createLedgerTemplate,
  createPlanAdapter,
  digestObject,
  discoverTargetRepository,
  emptyCalibrationQualityEvidence,
  fail,
  ...
  inspectPi,
  installRunnerSessionBinding,
  invariant,
  isSupportedNodeVersion,
  isSupportedPiVersion,
  jsonClone,
  loadCalibrationAttestation,
  loadCalibrationQualityEvidence,
  loadPi,
  loadRepositoryContext,
  makeCalibrationRunId,
  ...
  strictJsonClone,
  utcNow,
  validateCalibrationQualityEvidence,
  ...
  verifyRequiredLocalPaths,
  verifyRunnerOwnership,
  writeCalibrationQualityEvidence
};
