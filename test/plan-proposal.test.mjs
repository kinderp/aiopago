...
test("a stale or corrupt cooperative lock fails clearly without waiting or writing", () => {
  ...
  assert.throws(() => new PlanPort(x.path).apply(proposal(x.bytes, x.base)), (error) => error.code === "PLAN_LOCK_RECONCILIATION_REQUIRED" && error.message.includes("explicitly reconcile"));
  assert.deepEqual(readFileSync(x.path), x.bytes);
});
