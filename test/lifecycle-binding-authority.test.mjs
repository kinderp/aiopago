...
test("protected lifecycle authority extends the same canonical store without resume tables", () => {
  const x = fixture();
  try {
    assert.equal(requireSecureLifecycleAuthority(x.authority), x.authority);
    assert.equal(x.authority.status().schema, "aiopago.operation-authority/1.3.0");
    ...
    assert.equal(tables.has("authorizations"), false); assert.equal(tables.has("admissions"), false); assert.equal(tables.has("dispatch_attempts"), false);
...
test("bounded 1.2 protected store upgrade adds lifecycle schema and damaged current schema fails closed", () => {
  ...
  assert.equal(upgraded.status().schema, "aiopago.operation-authority/1.3.0"); upgraded.close();
