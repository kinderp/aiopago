...
| **0.2-E — UX interattiva unificata** | Projection condivisa nel TUI e handoff umano semplificato | **REMEDIATION / gate 0/2** dopo Independent Review BLOCKED su R1-M-13-REOPENED; entry operativa e bin main-only, nessun cleanup automatico di lock esistenti, package root senza runtime privilegiato, fresh-child boundary e Pi owned esatto 0.83.0; issue #30, draft PR #31 |
...
5. **0.2-E UX interattiva unificata: REMEDIATION / gate 0/2**, issue #30, draft PR #31, branch dedicato `feat/unified-human-ux-0.2-e`; dopo la prima correzione dell'import assoluto, il candidate `a13d48feacebdde4a2e9e76f7321b70fb69ebb54` ha avuto Fresh Independent Review Round 1 BLOCKED da R1-M-13-REOPENED perché un Worker rendeva il bundle operativo entry/main nello stesso PID. La nuova correzione lascia ogni JS spedito non privilegiato: il bundle operativo è lessicalmente dormiente e soltanto il fresh child sanitizzato creato dal bin aggiunge l'invocazione, senza affidarsi a `import.meta.main` o a indicatori Worker forgeabili. La remediation lock non autentica intrinsic JS: nessun lock esistente o recovery marker viene rimosso automaticamente da evidenza di processo; dead/unknown/crash-stale richiede riconciliazione umana. R1-M-07 conserva safety con contratto availability intenzionalmente ristretto. Il package root espone solo PlanPort e API read/data/validation/error, con zero Runner/Pi/CLI/storage/tracker/SafePoint/handoff authority. `aio` avvia il runtime operativo in un nuovo child sanitizzato. Pi 0.83.0 è una dependency esatta owned da Aiopago e ignora `NODE_PATH`, target/cwd e `PI_CODING_AGENT_ROOT`; redirect/missing/wrong version falliscono chiuso. Il tampering del tree installato trusted resta installation compromise fuori boundary. Tutti i finding storici restano richiesti chiusi. Gate 0/2.
