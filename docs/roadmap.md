...
| **0.2-E — UX interattiva unificata** | Projection condivisa nel TUI e handoff umano semplificato | **REMEDIATION / gate 0/2** dopo Fresh Round 1 BLOCKED su R1-H-12/R1-M-12/R1-L-10; cleanup Windows indipendente dal wrapper `process.kill` e Pi privilegiato da slot npm fisici deterministici; issue #30, draft PR #31 |
...
5. **0.2-E UX interattiva unificata: REMEDIATION / gate 0/2**, issue #30, draft PR #31, branch dedicato `feat/unified-human-ux-0.2-e`; il candidate `178294392b61116ec596f1b7eae4d590f818a97c` ha avuto Fresh Round 1 BLOCKED da R1-H-12/R1-M-12/R1-L-10. La remediation conserva schema lock 0.3.0, nonce, recovery hard-link e ABA, ma su Windows autorizza cleanup soltanto con `ESRCH` dal binding Node nativo catturato e validato, mai dal wrapper mutabile `process.kill`; PowerShell resta solo diagnostica/LIVE. Il PID reuse ambiguo resta fail-closed; un owner realmente terminato recupera via native absence. `GuardianRunner.create()` carica Pi/pi-ai solo dagli slot npm fisici deterministici relativi ad Aiopago, rifiuta junction/symlink e ignora `NODE_PATH`, cwd target e `PI_CODING_AGENT_ROOT` sul percorso privilegiato; il relativo errore non raccomanda più l'override ignorato. Injection diretta e supporto test impacchettato restano chiusi. R1-H-11/R1-M-11, R1-L-09, R1-M-01..M-08, R2 e la catena storica restano chiusi. Gate 0/2.
