...
| **0.2-E — UX interattiva unificata** | Projection condivisa nel TUI e handoff umano semplificato | **REMEDIATION / gate 0/2** dopo Fresh Round 1 BLOCKED su R1-H-11/R1-M-11; cleanup Windows solo con native ESRCH e Pi privilegiato selezionato da Aiopago; issue #30, draft PR #31 |
...
5. **0.2-E UX interattiva unificata: REMEDIATION / gate 0/2**, issue #30, draft PR #31, branch dedicato `feat/unified-human-ux-0.2-e`; il candidate `c32e1048af7c6af150b254ec514e1c65ce216cc6` ha avuto Fresh Round 1 BLOCKED da R1-H-11/R1-M-11. La remediation conserva schema lock 0.3.0, nonce, recovery hard-link e ABA, ma su Windows autorizza cleanup soltanto con un nuovo `process.kill(pid,0)` che restituisce `ESRCH`: PowerShell è solo diagnostica/LIVE, mentre DEAD, identity mismatch, PATH/PATHEXT/cwd shadow, assenza, timeout e output malformato sono `UNKNOWN`. Il PID reuse ambiguo resta fail-closed; un owner realmente terminato recupera via native absence. `GuardianRunner.create()` carica il Pi trusted installato e rifiuta `options.pi` e runtime dependency injection prima di mutation; il supporto Pi dei test sorgente è rimosso fisicamente dal bundle. Callback read-only residue non ricevono Runner/factory/storage/tracker. R1-L-09, R1-M-01..M-08, R2 e la catena storica restano chiusi. Gate 0/2.
