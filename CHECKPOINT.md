# CHECKPOINT — AIOPAGO R1-M-13 RESUME AUTHORIZATION / ADMISSION / DISPATCH AUTHORITY MIGRATION

- **Starting integrity:** local HEAD, tracked upstream, and fresh `git ls-remote` were `cbf09589b3f9161a1030c25c0a6f65d04da25dbc`; branch `feat/unified-human-ux-0.2-e`; merge-base `6386d599e8510933786ea34822b01adac2fdb205`; clean starting tree; PR #31 OPEN/DRAFT; independent gate 0/2. No merge, mark-ready, force push, 0.2-F, generic Human Action Broker, general recovery, plan authority, or artifact authority migration occurred.
- **Recovered taxonomy/contract:** invocation-local Pi/TUI YES/NO is not reusable consent. Protected `resume_readiness` binds the exact reservation digest, target/Runner/binding/incarnation, latch, prompt bytes/digest, plan semantic digest and referenced artifact digests. Durable authorization, admission, dispatch intent/attempt and outcome remain distinct objects. The exact historical project objects were `authorizations`, `admissions`, `dispatch_attempts`, handoff projection fields and resume journal events; their writers/readers/fields/transitions/security classification are inventoried in `docs/0.2-e-resume-admission-dispatch-authority.md`.
- **YES/NO semantics:** NO creates no authorization, admission, dispatch intent, external call, or latch release. Exact NO replay is idempotent; same request changed to YES conflicts. A fresh NO→YES interaction is allowed. YES→NO cannot revoke committed authority. Exact or different-request duplicate YES returns the same canonical identities with no dispatch permit; changed actor/payload conflicts. No generic consent token, auto-resume, toggle, or cross-handoff authorization exists.
- **Protected schema/protocol:** the same protected `%ProgramData%\Aiopago\OperationAuthority\canonical\operations.sqlite` advances to `aiopago.operation-authority/1.4.0`; private inherited P1S/P2 protocol advances to `/5`. New tables are `resume_readiness`, `resume_authorizations`, `resume_admissions`, `resume_dispatch_attempts`, and append-only `resume_authority_events`; shared `authority_requests` remains the request conflict ledger. Fixed operations only: readiness commit, decision, outcome, get, events. P1S remains LocalService plus unique service SID; P2 remains protected Node over inherited pipes; no portable fallback.
- **Atomicity:** one protected `BEGIN IMMEDIATE` revalidates exact protected reservation, ACTIVE binding/incarnation, engaged non-takeover latch and empty resume state, then releases the latch and inserts authorization, admission and attempt-1 `DISPATCHING` intent plus events/request result. Authorization-only and admission-without-intent committed seams are structurally unreachable. Forced admission/intent failures and physical process exit before COMMIT restored exact readiness, engaged latch and zero auth/admission/intent rows.
- **Admission exactly-once:** six independent protected clients competed on one YES; exactly one received the external dispatch permit, five received the same idempotent canonical result, and durable counts remained 1 authorization / 1 admission / 1 intent. Distinct identities can progress independently; stale latch, stale readiness, stale/superseded lifecycle, same-ID ABA, request conflict and changed actor reject. No second external creation/send is permitted by a duplicate or restart.
- **Dispatch contract:** exact claim is exactly-once protected admission, exactly-once durable intent, and at-most-once invocation by this protected workflow. Pi 0.83.0 `sendUserMessage()` exposes no stable idempotency primitive, so externally exactly-once session/provider execution is **not claimed**. Intent exists before invocation. A post-invocation exception is conservatively `UNKNOWN`. Crash after external success but before outcome commit leaves `DISPATCHING`; restart grants no replay permit. A terminal outcome is one-way and contradictory reports conflict.
- **Takeover/lifecycle races:** takeover or shutdown/supersession first refuses authorization/admission/intent. Atomic YES/intent first is the opposite SQLite ordering. A final synchronous protected latch/binding/current-target reread before the call detects takeover or shutdown after admission/intent and records `FAILED` with zero external calls. Once external invocation begins, ambiguous errors are `UNKNOWN`. Same textual target ID at a new incarnation cannot satisfy old readiness.
- **Genuine Pi YES/NO:** genuine `@earendil-works/pi-coding-agent@0.83.0` completed protected NO then fresh YES for handoff `HO-5ff8845a2a967020ec21954c`, binding `BIND-4a52918a-6ab2-4b0d-a4aa-7fe935dc7262`, authorization `AUTH-fd8bc11e2285d5c42e5802ed`, admission `ADM-fd8bc11e2285d5c42e5802ed`, dispatch `DSP-f44f3f5fc02c7ad78103e209`, one real instrumented external call and canonical `ACKNOWLEDGED`. NO had zero calls/authority; duplicate after success had zero additional calls. Secure status facade displayed canonical `AUTHORIZED / COMMITTED / ACKNOWLEDGED`, not project rows.
- **Crash matrix:** A authorization-before-admission is unreachable/atomic. B committed admission+intent before call survives as `DISPATCHING` and is not replayed. C exception during/after request becomes `UNKNOWN`, one call. D external success before outcome commit remains `DISPATCHING`, one call, zero retry, reconciliation required. E outcome commit before project projection remains canonical `ACKNOWLEDGED`; projection is healed from canonical state with zero external replay.
- **Direct P0/false negative:** real Explorer-derived medium P0 wrote plausible later project YES, admission and ACK success for `HO-FORGED-BY-P0`; protected readiness/auth/admission/dispatch were absent and secure progress was zero. After legitimate protected success, P0 deleted real project rows/handoff and wrote NO plus fake FAILED/newer target/Runner/binding; protected authorization/admission/ACK outcome, reservation, active source and lifecycle remained exact.
- **Activated source:** genuine Pi 0.83.0, factory 1, commands 4, handlers 10, Runner authority captured, portable `OP-FORGED` and portable takeover present, installed bytes unchanged. Protected authorization/admission/dispatch delta was zero; protected operation/latch/handoff/lifecycle deltas remained zero. Result PASS for all five migrated domains.
- **Restart/recovery boundary:** readiness-only, admission/intent, ACK, `FAILED`, `UNKNOWN` and ambiguous `DISPATCHING` survive P1S/P2 restart without project input. Project manipulation while down has no canonical effect. Ambiguous state stops automatically. **RECOVERY AUTHORITY REMAINS UNMIGRATED.** No broad reconciliation was implemented.
- **Plan/artifact boundary:** protected readiness stores exact prompt bytes and binds reservation plan semantic digest plus checkpoint/manifest references used by this operation. This does not make `TASK_PLAN.md`, checkpoint bytes, manifest bytes, or artifacts generally protected authentic. Plan and artifact authority remain unmigrated; no claim beyond exact resume control authority is made.
- **Physical Windows oracle:** elevated scoped service `AiopagoOperationAuthorityTest-16380-1788068970729`; SID `S-1-5-80-3434175146-572143116-2153400451-2328437716-2609459984`; broker/Node/worker SHA-256 `6e3b424045b0b68cb6518f34312726d65fec87e3bf3b6c0e73bfcc16950c2500` / `995a3fb3cefad590cd3f4b321532a4b9582fb9c6575320ed2e3e894caac3e362` / `d793cfc2f393e51fff67c8528631de1651e4ded44f1d6a039ae713812770fe73`. Protected NO/YES/duplicate/outcome, restart, both resume crash seams, P0 attacks, activated source, schema/protocol/hash/timeout failures and all prior domains passed.
- **Cross-service/fail closed:** second unrestricted LocalService had S-1-5-19 but not the Aiopago SID; 21/21 canonical/latch/handoff/lifecycle/resume/admission/intent/outcome/key/ACL/broker attempts were denied. Missing/stopped service, missing DB, missing/corrupt current schema including resume schema, identity mismatch, protocol mismatch, worker hash mismatch, channel timeout/EOF, stale handoff/binding/incarnation/latch, request conflict, already-admitted conflict and dispatch ambiguity all failed closed without project fallback.
- **Regression/tests:** protected operation+latch+handoff+lifecycle+resume **52/52**; trusted handoff/race/crash **80/80**; genuine Pi **154/154**; core+human+unified **108/108**; takeover/lock **35/35**; portable real-Pi handoff **1/1**; package routes **7/7** when isolated. Distinct focused total **437/437 PASS**. Activated-source and elevated physical oracle PASS. `npm run check` PASS with brand guard and 77 modules; `git diff --check` PASS. One initial package aggregate exceeded its 600-second harness timeout and one deliberately parallel pack run collided in the shared `dist` build; all seven routes passed independently in fresh processes and the final serialized build/pack passed.
- **Package/security:** actual `npm pack --json`: 13 files, 221,335 packed / 968,170 unpacked bytes. Only existing three bundles plus allowlisted bins/docs/metadata. No source, tests, test support, PoC, C# service source/binary, map, ProgramData state, SQLite, key, secret, or tarball retained. Strict source/diff inspection found no credential/private-key/capability leakage.
- **Cleanup:** authority and sentinel services removed (SCM query absent); scoped ProgramData/public/temp roots, generated native binaries, P1S/P2/sentinel processes, handles/pipes, tarballs and identity material removed. Ignored `.guardian` logs contain bounded non-secret local evidence and are not staged.
- **Findings:** HIGH none; MEDIUM none; LOW none. INFORMATIONAL: no external Pi idempotency primitive, so ambiguous outcome requires future recovery; plan/artifact authenticity remains a separate domain; package Worker teardown/aggregate duration remains the known Node 22.19 test-harness behavior.
- **Status/next:** operation, latch/takeover, handoff reservation/active-source, lifecycle/session binding, and resume authorization/admission/dispatch are MIGRATED/PASS. R1-M-13 remains OPEN for recovery, plan, artifact and any remaining privileged authority. Recommended next bounded domain: **RECOVERY AUTHORITY**, unless its inventory proves plan/artifact authenticity is a prerequisite; then select **PLAN / ARTIFACT AUTHORITY** first. Do not implement either here.
- **Commit/push:** one coherent intermediate commit is authorized after explicit staging. Immutable SHA, normal push, remote head and clean tree are post-commit report facts. PR remains OPEN/DRAFT; gate remains 0/2.
...
NEXT: RECOVERY AUTHORITY INVENTORY
DO NOT MERGE
DO NOT START 0.2-F
```

**Suggested next session:** `aiopago-r1-m-13-recovery-authority-inventory`

**Minimum resume prompt:** Continue Aiopago 0.2-E R1-M-13 on `feat/unified-human-ux-0.2-e`. First verify local/remote HEAD equals the exact resume-authority commit reported by the prior session, worktree clean, merge-base `6386d599e8510933786ea34822b01adac2fdb205`, PR #31 OPEN/DRAFT and gate 0/2. Read operational rules, latest checkpoint, all five authority documents, and inventory only remaining recovery authority edges. Select one bounded recovery slice only if it does not require simultaneous plan/artifact authenticity; otherwise stop and recommend plan/artifact authority. Do not merge/mark-ready, do not start 0.2-F, and do not repeat the completed five-domain physical oracle without a relevant change.

# CHECKPOINT — AIOPAGO 0.2-E R1-M-13 DISTINCT-IDENTITY RESTART/CRASH FOUNDATION POC PASS

- **Starting integrity:** local HEAD and `origin/feat/unified-human-ux-0.2-e` both remained `7ef2d831caaa16c5cf6ce44edae0c671a7e20e6f`; merge-base remained `6386d599e8510933786ea34822b01adac2fdb205`; branch remained `feat/unified-human-ux-0.2-e`; PR #31 remains **OPEN / DRAFT**; independent gate remains **0/2**. Existing unstaged architecture/reproducer/PoC work was inspected and preserved. No reset, clean, stash, checkout-over, merge, mark-ready, production candidate, or 0.2-F work occurred.
- **Elevation proof:** elevated probe PID **23636** ran as `LAPTOP-NMCGI8PV\\antonio`, SID `S-1-5-21-1268775888-3906723769-323924214-1001`, high integrity `S-1-16-12288`, with Administrators enabled; `net session` exited 0. A unique SCM preflight create/query/delete returned 0/0/0 and post-delete query 1060. `AiopagoR1M13Poc` did not pre-exist.
- **Protected installation:** deterministic final Go broker SHA-256 **`bf74836fda78697fe632e20cb089f951ae6240ec323c1c14f90c8f519d8430e2`** matched before/after copy and all attacks at `C:\\ProgramData\\Aiopago\\R1M13Poc\\bin\\broker-service.exe`. Protected parent/root/bin/canonical/control/test-output ownership was explicitly corrected to `BUILTIN\\Administrators`; DACLs grant SYSTEM, Administrators and only the named service SID as needed. Shared LocalService, invoking user, Users and Authenticated Users have no protected write grant. Public projection was separate at `C:\\ProgramData\\Aiopago\\R1M13PocPublic`.
- **Service identity:** P1S ran as `NT AUTHORITY\\LocalService`, SID `S-1-5-19`, not the invoking user and not LocalSystem. Enabled service SID was `NT SERVICE\\AiopagoR1M13Poc`, `S-1-5-80-1929571751-1865767387-1503916280-2855656575-2487379996`; service token integrity was `S-1-16-16384`. `RESTRICTED` SID type was attempted first, but protected Node and `whoami.exe` children failed loader initialization with `0xc0000142`; no Windows system ACL was weakened. Passing execution used `UNRESTRICTED` SID type while protected DACLs continued to grant only the unique service SID, never shared `S-1-5-19`.
- **Real P0 / P2 proof:** Explorer PID **13704** supplied the actual filtered primary token through native token duplication. Primary P0 PID **37672** reported the historical interactive SID, medium integrity `S-1-16-8192`, and Administrators deny-only; supplemental medium P0 PIDs **24856, 34080, 16596, 37932** covered ACL/replay/access/owner-hardening probes. P1S PIDs before/after were **37932 / 28668**; P2 PIDs were **35104 / 7612** and the private protocol enforced P2 parent/PID binding. P1S-created inherited anonymous pipes carried CAP_priv; capability was absent argv/environment and no named semantic endpoint existed.
- **Persistent authority:** P1S generated a raw Ed25519 authority only after service entry under protected `canonical`; no private bytes were logged or retained. Public fingerprint before/after was stable at **`d33e4cb16aa25db116145190b004a981ba34cbc47875fe4560c53c080901bcf1`**. Public projection was never canonical input.
- **Authorized chain:** sequence 1 `OP-POC-BEFORE-RESTART` payload digest `c4bee84ee5ac5b8b1c5b167629206617c6a001d7e965146900898dc842f32cf3`, record digest `fd76d4dd45ff5dac4e5a4d517c4eb7a80c80fa2b4712411c0b3e9cae19f50c0e`; sequence 2 `OP-POC-AFTER-RESTART` payload digest `26343f91484ef07618cbb9c63da52cd391c796754fde86fe78e731d2efa479cc`, previous digest equal to record 1, record digest `52eaba55a5619e1f2d21ae51a58b2980dab3cd787047f9cca749a239eaaf47e1`.
- **P0 tamper matrix:** actual medium-token operations denied key read/copy/use/delete/replace/rename/DACL; canonical read/write/delete/replace/rename/directory-delete/DACL; protected root enumeration/delete/DACL; broker read/overwrite/same-path write/rename/replace/bin-child/DACL; and service binary-path/account/SID-type/DACL/delete/start changes (`EPERM` or Win32 exit 5). P0-created public junctions could not access or rename over protected canonical/bin targets. P0 could query service lifecycle and could forge/delete/replace projections, as designed. Owner hardening was separately verified: parent/root/key/canonical/binary DACL changes all returned exit 5.
- **Restart authenticity:** broker loaded protected sequence 1 despite attacker projection sequence 999 and attacker `OP-FORGED-BY-P0.json`; projection-consumed-as-canonical remained false; forged canonical count was 0; post-restart authorized mutation reached sequence 2; P0 direct privileged mutation count was **0**. Exact duplicate returned `IDEMPOTENT_RECORDED_RESULT`; same ID/changed payload returned `REQUEST_ID_CONFLICT`.
- **Crash oracle:** P1S PID **35216** terminated abruptly after complete flushed `state.json.next` write and before atomic replace; SCM reported 1067. Pre/post-crash canonical SHA was exactly `1f7174f0ccf1be4142c16e24aa69ebaae06bf1ef4648f5f9753179df1bde61d5` at sequence 2. Recovery PID **2252** found the structurally valid uncommitted temp (`a55c17e9…11dc8`), did not admit it, removed it, and retained exactly the two committed records. No P2 remained. Exact older signed sequence-1 projection replay was ignored.
- **Confused deputy:** public surface was SCM lifecycle/query plus writable projection files only. Medium P0's fabricated `OP-FORGED-BY-P0` file and `sc start` attempt produced no broker mutation; SCM start returned exit 5. No public mutation RPC/pipe existed.
- **Important corrected finding:** installer-created protected objects initially retained invoking-user ownership even with restrictive DACLs. Because owner authority can permit DACL rewrite, final provisioning explicitly sets Administrators ownership before P0 testing. Medium P0 then proved parent/root/object ACL changes denied. This is mandatory in any production provisioning.
- **Cleanup:** exact PoC service stopped/deleted; query after delete **1060**; protected root, public root and newly created `C:\\ProgramData\\Aiopago` parent removed; broker/P2 process count 0; handles/pipes closed; no key/secret retained. The Defender-blocked phantom `service-setup-preflight.ps1` remains inaccessible (`Test-Path` false while Git enumeration still reports it); no Defender bypass was attempted and it must not be staged.
- **Focused validation:** Go 1.24.6 `fmt`/`vet` and deterministic service/medium-launcher builds PASS; service build reproduced exact tested SHA; Node syntax and evidence JSON PASS; PowerShell parser PASS; clean restart/crash/replay/confused-deputy/cleanup oracles PASS; `git diff --check` PASS. npm package inventory excludes all PoC/R1-M-13 material. Full 950-test suite intentionally not run because production runtime was not migrated.
- **Cross-platform / product modes:** Windows physical foundation **PASS**. Linux dedicated systemd identity/protected `/var/lib/aiopago` and macOS dedicated launchd identity/protected state remain **DESIGN ONLY**. SECURE AUTHORITY vs PORTABLE/DEV mode split remains recommendation only; no product mode was implemented.
- **Scope and gate:** current production SQLite/files remain ordinary-user writable; no GuardianStorage, PlanRevisionWriter, handoff, takeover, recovery, lifecycle, journal or tracker production writer migrated. **R1-M-13 REMAINS OPEN. INDEPENDENT GATE 0/2. PR #31 DRAFT. DO NOT MERGE. DO NOT START 0.2-F.**
- **Foundation verdict / exact next work unit:** **PERSISTENT AUTHORITY RESTART POC: PASS**. Next: **ONE RESTART-AUTHENTIC PRODUCTION MUTATION**, recommended operation journal / `ToolOperationTracker`; do not migrate it in this completed PoC session.

# CHECKPOINT — AIOPAGO 0.2-E R1-M-13 PERSISTENT AUTHORITY / OS SETUP BLOCKED

- **Starting integrity:** local HEAD and remote `origin/feat/unified-human-ux-0.2-e` remained `7ef2d831caaa16c5cf6ce44edae0c671a7e20e6f`; merge-base remained `6386d599e8510933786ea34822b01adac2fdb205`; branch remained `feat/unified-human-ux-0.2-e`; PR #31 remains **OPEN / DRAFT**; independent gate remains **0/2**. Existing unstaged spike, reproducer, native-capability PoC and checkpoint were inspected and preserved. No reset/clean/stash/checkout-over occurred.
- **Same-user execution:** added non-production `poc/r1-m-13-persistent-authority/{same-user-anchor.ps1,same-user-anchor.test.mjs,windows-setup-preflight.mjs,README.md}` and `docs/0.2-e-r1-m-13-persistent-authority.md`. Windows x64 P0 harness PID **22836** ran each attack in a distinct fresh same-user PowerShell process under `LAPTOP-NMCGI8PV\\antonio`, SID `S-1-5-21-1268775888-3906723769-323924214-1001`.
- **NTFS / DPAPI / credentials:** a current-user-only NTFS ACL file was readable, writable, deletable, replaceable and its ACL was changeable by P0. A separate process recovered the exact DPAPI CurrentUser cleartext and replaced its ciphertext. Windows current-user PasswordVault/Credential Locker likewise returned the exact password and permitted delete/replacement. All persist across process restart and all are rejected as the same logon authority.
- **CNG software key:** P0 could not export a persisted non-exportable ECDSA P-256 private key, but opened it by name through Microsoft Software Key Storage Provider, produced a 64-byte signature, deleted it and recreated the same name. Fingerprint changed from `82700a5db791acbb69d89b255ccafeca5e54c8e67cfc966f03d83d0596bfbcfc` to `9eb68e40757521c9e71d2ad0793ca1b6ca5c6fd3e75a7c11a97c8cc2b1351e7b`. Non-exportability is not caller authentication.
- **TPM CNG key:** Microsoft Platform Crypto Provider was available. Its persisted non-exportable ECDSA key was also opened and used by separate same-user P0 PID **25948** to produce a 64-byte signature, then deleted/recreated. Fingerprint changed from `a53de837937298a127bc7d5fddbaef2824eebcd668f8d7e047deb3e199a9e534` to `7053bb9993927b23781d64ee0783f4a338aef883fbe7e7a2961df005c1211344`. TPM protection prevents export but does not distinguish arbitrary same-user code.
- **Formal decision:** **PERSISTENT AUTHORITY REQUIRES A DISTINCT OS SECURITY IDENTITY**. Invoking-user service, ordinary ACL/DPAPI/Credential Locker, software CNG, TPM CNG and CurrentUser certificate-store variants cannot provide the required P0 distinction.
- **Selected Windows model:** native P1S service under least-privileged **LocalService plus restricted `NT SERVICE\\AiopagoR1M13Poc` service SID**; protected `%ProgramData%\\Aiopago\\R1M13Poc` ACL grants only that service SID, SYSTEM and Administrators. P1S creates a persistent Ed25519 key and canonical sequence/request/payload-digest/previous-digest/signature chain. Public project state is only a disposable projection. NetworkService and LocalSystem are unnecessary; LocalService without service-specific ACL is shared and insufficient.
- **IPC/confused deputy:** SCM/startup control is not semantic mutation RPC. The target bounded service creates a private inherited P1S→P2 channel and per-session capability for one fixed sentinel operation; P0 receives neither and cannot name the mutation on control. No eval/module/method/general writer exists. Human/CLI spawning is documented as durable-authority scope, not a general endpoint-malware claim.
- **Setup blocker:** preflight PID **31904** proved the token is not elevated; real `net.exe session` administrator access check returned Windows error 5 / access denied. A uniquely named direct `sc.exe create` rights probe failed at `OpenSCManager` with exit/error 5; query returned 1060, confirming no probe service was created. This environment cannot create the temporary SCM service, LocalService token, restricted service SID or protected root. No same-user simulation or weakened ACL was substituted. Exact elevated `sc.exe`/`icacls` setup, verification, attack and cleanup commands are in the PoC README.
- **Restart/crash oracle:** **NOT EXECUTED** because the required identity setup is unavailable. Broker/P2 PIDs, stable service key fingerprint, protected canonical tamper results, clean restart and forced-crash recovery therefore remain N/A rather than simulated PASS. The target atomic rule is complete temp+flush+replace; projection follows canonical commit. Production WAL/anti-rollback/recovery remains later work.
- **Cross-platform/packaging:** Windows physical test remains next. Linux dedicated systemd user plus `/var/lib/aiopago` and socketpair/pipes, and macOS launchd daemon user plus protected Application Support and pipe/socketpair/XPC mapping are **DESIGN ONLY**. Retain meta npm package plus exact platform packages and add explicit elevated signed broker provisioning; no network postinstall download and no silent elevated npm postinstall. Recommend separately labeled SECURE AUTHORITY and PORTABLE/DEV modes; not implemented.
- **Scope:** explicit **0.2-E SECURITY/FOUNDATION — R1-M-13 PERSISTENT AUTHORITY** only. No GuardianStorage, PlanRevisionWriter, handoff, takeover, recovery or lifecycle writer migrated. No production code/candidate, commit, push, merge or mark-ready. 0.2-F remains untouched.
- **Focused validation:** same-user anchor harness PASS; TPM available and rejected; service preflight correctly BLOCKED; Node syntax and PowerShell parser PASS. Full production suite intentionally not run.
- **Tooling note:** an initial P/Invoke-based preflight draft triggered Windows Defender (`ERROR_VIRUS_INFECTED`) and is inaccessible as untracked `poc/r1-m-13-persistent-authority/service-setup-preflight.ps1`; it was replaced by safe `windows-setup-preflight.mjs`. Defender has not released the quarantined path for deletion in this session. It is outside package files and must not be staged/committed; remove it after Defender releases the handle/quarantine.
- **Final decision:** **PERSISTENT AUTHORITY POC: BLOCKED_BY_REQUIRED_OS_SETUP**. R1-M-13 remains open, gate 0/2, PR #31 draft. **DO NOT MERGE. DO NOT START 0.2-F.**
- **Suggested next session name:** `Aiopago R1-M-13 elevated service restart oracle`.
- **Minimum resume prompt:** “Continue Aiopago 0.2-E security/foundation R1-M-13 persistent authority on `feat/unified-human-ux-0.2-e`. First verify local/remote HEAD remains `7ef2d831caaa16c5cf6ce44edae0c671a7e20e6f`, inspect operational rules, `CHECKPOINT.md`, `docs/0.2-e-r1-m-13-persistent-authority.md`, both R1-M-13 PoC directories, status and diff, and preserve all unstaged work. Remove the Defender-blocked untracked preflight draft only after Windows releases it. With an actual elevated Administrator terminal, implement/review the bounded native service and execute the README's LocalService + restricted service-SID clean restart/crash/tamper oracle; do not simulate identity separation or weaken ACLs. Do not migrate production writers unless restart PASS; do not merge/mark-ready PR #31, do not claim closure, and do not start 0.2-F. Gate remains 0/2.”

# CHECKPOINT — AIOPAGO 0.2-E R1-M-13 NATIVE CAPABILITY BOUNDARY DECISION / POC BLOCKED

- **Starting integrity:** local HEAD and remote branch remained `7ef2d831caaa16c5cf6ce44edae0c671a7e20e6f`; merge-base remained `6386d599e8510933786ea34822b01adac2fdb205`; branch `feat/unified-human-ux-0.2-e`; PR #31 **OPEN / DRAFT**; independent gate **0/2**. Existing unstaged `CHECKPOINT.md`, `docs/0.2-e-r1-m-13-process-capability-spike.md`, and `test/reproducers/r1-m-13-activated-source.mjs` were inspected first and preserved. No reset/clean/stash occurred.
- **Architecture inventory:** journal, all GuardianStorage tables, tracker operations, plan coordination/writes, handoff reservation, takeover latch, authorization/admission/dispatch, resume, recovery, lifecycle/bindings, artifacts and metrics were traced to `runner.mjs`, `storage.mjs`, `safety.mjs`, `handoff.mjs`, `handoff-plan-internal.mjs`, `ledger.mjs`, `plan-store.mjs`, `artifact-store.mjs` and Pi integration. Every durable domain has a direct same-user SQLite/filesystem bypass today; lexical JS closure/package boundaries do not authenticate bytes.
- **Selected process direction:** native capability launcher plus mutation broker remains preferred over an administrator-installed permanent service for on-demand portability. P0 is public/hostile JS, P1 is native broker, P2 is privileged Node/Pi. P1 must be the actual user-facing executable; hostile JS correctly calling `spawn` cannot be the trust root. A plain launcher that starts the current Runner is insufficient.
- **Bounded PoC:** added non-production `poc/r1-m-13-native-capability/{broker.go,p2-runtime.mjs,poc.test.mjs,README.md}` outside the npm allowlist. P1 is CGO-free Go; it verifies fixed P2 source identity, selects build-bound Node independently of P0 `process.execPath`, sanitizes environment, creates CAP_priv with `crypto/rand` after native entry, and transfers it only over P1-created inherited anonymous pipes. The semantic protocol is fixed to HELLO/SESSION_BIND/AUTHORIZE_MUTATION/MUTATION_RESULT/SHUTDOWN with version, request ID, operation type, bounded payload and explicit result; no eval/module/function/method/writer dispatch.
- **Process evidence:** executed Windows x64 graph `P0=8604 → P1=16340 → P2=23212`; all PIDs distinct and P2 parent was P1. CAP_priv was 32 random bytes, absent package files/argv/environment/P0, and present only in P1/P2 for the channel lifetime. P2 exited and private endpoints closed.
- **Positive/negative result:** one fixed test sentinel `POC_OPERATION_TERMINAL / OP-POC-AUTHORIZED` was broker-accepted. Unauthorized accepted mutation count was **0**. Direct equivalent state rewrite, absolute import, Worker, copied source, data URL, temporary module, same-PID genuine Pi patch, mutable `process.execPath`, spawn/spawnSync+CJS/ESM patches, env/argv, fake capability, replay, duplicate and changed-payload request-ID attacks all created zero unauthorized mutation. Duplicate was idempotent with zero new record.
- **Exact exploit preserved:** reran `test/reproducers/r1-m-13-activated-source.mjs`; genuine Pi **0.83.0**, factory **1**, commands **4**, handlers **10**, Runner authority **1**, `OP-FORGED` = `TERMINAL / KNOWN_SUCCESS / READ_ONLY`, `HUMAN_TAKEOVER` = **1**, installed bytes unchanged. The native PoC does not suppress the original vulnerability.
- **Durable-state blocker:** while P1 remained live, P0 replaced the sentinel with `OP-FORGED-BY-P0` while retaining the genuine MAC; P1 rejected/restored it and unauthorized count stayed 0. After P1 exits, a later portable P1 lacks the old ephemeral capability. A same-directory public key can be replaced with state, a static/package key is forbidden, and same-user files/credential storage do not distinguish P0. Restart result: **`UNVERIFIABLE_WITHOUT_PERSISTENT_TRUST_ANCHOR`**. Existing resume/recovery require cross-process durable acceptance, so the architecture is incomplete.
- **Technology evidence:** repository initially had no Go/Rust/C/C++/Zig toolchain; .NET existed but adds runtime/NativeAOT complexity. Official Go 1.24.6 was unpacked only under OS temp. `gofmt`/`go vet` passed. CGO-free stripped cross-builds produced Windows x64 PE 2,516,480 bytes, Linux x64 statically linked ELF 2,408,596 bytes, and macOS arm64 Mach-O 2,296,610 bytes. Only Windows was executed; cross-build output is not Linux/macOS runtime proof. No binary was added to the worktree.
- **Packaging direction:** recommend deterministic meta-package plus exact platform packages for Windows x64, Linux x64 and macOS arm64, with an install/bin prototype proving the shell reaches native P1 without an invocation-time JS predicate. Reject postinstall network download. Signing/notarization, provenance, version coupling and unsupported-platform behavior remain release work.
- **Interactive gap:** P2 must retain genuine Pi/TUI, terminal and session ownership. Production requires a dedicated private broker channel plus terminal stdin/stdout/stderr, resize, signal/Ctrl-C, exit/crash and Job Object/process-group behavior. The PoC uses P2 stdio for protocol and proves cleanup only; it does not yet prove interactive equivalence or Linux/macOS execution.
- **Scope/roadmap:** production migration cannot fit silently in existing 0.2-E. Add an explicit **0.2-E security/foundation work unit — R1-M-13 Capability Boundary**, covering persistent authority decision, native/terminal foundation, one restart-authentic durable primitive, all writers, Runner lifecycle, platform bins, legacy removal and full adversarial gate. This is not a new public version. **0.2-F durable stop/resume remains untouched and not started.**
- **Focused validation:** native build PASS; positive sentinel PASS; focused 16-entry attack matrix PASS for live broker; resource cleanup PASS; exact activated-source vulnerability reproduced; Go static/cross-build checks PASS; `npm pack --dry-run --json` remained 12 files and excluded PoC/spike/reproducer; `git diff --check` and JS syntax checks PASS. Full suite intentionally not run because production code did not change and architecture remains blocked.
- **Production/PR state:** no production implementation, candidate commit, push, merge or mark-ready. PR #31 remains **OPEN / DRAFT** and gate remains **0/2**.
- **Final decision:** **NATIVE CAPABILITY ARCHITECTURE POC: BLOCKED**. The ephemeral process boundary is real, but the minimum next architectural decision is an OS-protected cross-platform persistent trust anchor versus a protected long-lived broker/service, followed by restart-authenticity and dedicated interactive-channel proof. **R1-M-13 ARCHITECTURAL BLOCKER REMAINS. DO NOT MERGE. DO NOT START 0.2-F.**

# CHECKPOINT — AIOPAGO 0.2-E R1-M-13 PROCESS-CAPABILITY FEASIBILITY / ARCHITECTURAL BLOCKER

- **Starting integrity:** previous candidate `7ef2d831caaa16c5cf6ce44edae0c671a7e20e6f` on `feat/unified-human-ux-0.2-e`; local and `origin/feat/unified-human-ux-0.2-e` matched exactly, merge-base remained `6386d599e8510933786ea34822b01adac2fdb205`, the worktree was clean, and PR #31 was **OPEN / DRAFT**. Fresh Independent Review Round 1 was **BLOCKED** by `R1-M-13-REOPENED`; gate remains **0/2**.
- **Exact activated-source reproduction:** `test/reproducers/r1-m-13-activated-source.mjs` packs and installs the untouched candidate, initializes with genuine `@earendil-works/pi-coding-agent@0.83.0`, reads installed `dist/cli-entry.mjs`, applies the exact public physical-marker replacement from `bin/aio.mjs`, appends `await aiopagoOperationalEntrypoint()`, and imports an attacker-owned base64 data URL in the existing consumer PID. Installed bytes and SHA-256 stayed exact (`c024abf40d187705a2d0defdab1f18c2da7e286a637658c51ca289264f823716`). Observed factory **1**, commands **4**, handlers **10**, Runner authority **1**, `OP-FORGED` = **TERMINAL / KNOWN_SUCCESS / READ_ONLY**, and durable `HUMAN_TAKEOVER` = **present**. Result: **R1-M-13 reproduced**.
- **Threat-model determination:** arbitrary public JavaScript, readable installed artifacts, attacker-owned copies/transforms, data-URL/vm/Worker/temporary-module evaluation, and same-PID genuine-Pi patching remain in scope. Physical modification of the installed trusted tree remains historically outside the boundary. A deliberately attacker-preloaded new process remains the existing process-compromise exclusion. Activated-source reconstruction is in scope; the contrary exclusion first introduced at `7ef2d83` is withdrawn and was not used for this decision.
- **Fundamental answer:** the legitimate operational child currently has no capability an arbitrary same-PID consumer cannot reconstruct. Its apparent distinctions are a public source transform, lexical invocation, sanitized environment, argv, physical-location marker and fresh PID. No OS-backed authority is bound to that PID, and the bundle contains self-contained Pi/Runner/filesystem/SQLite mutation code.
- **Pure-JS decision:** impossible under the established attacker model. Function/closure/WeakMap/Symbol/private-field/source-marker/import-meta/Worker/process/env/argv/package-export checks remain transformable JavaScript. If such a check guards a self-contained writer, an attacker-owned copy removes the check. No fourth JavaScript predicate was implemented.
- **Out-of-process decision:** an ordinary Node child or unauthenticated same-user service is insufficient because hostile parent JS controls bootstrap primitives and an unrestricted service is a confused deputy. A service is security-capable only when trusted from process start, authenticated through an OS-backed private channel/identity, and the public PID receives only strict serialized intent/results while all mutation is externally enforced.
- **Native/private launcher decision:** a native P1 can create an anonymous inherited pipe/handle and dynamic `CAP_priv` after leaving P0, sanitize before Node initialization, and avoid mutable JavaScript `process.execPath`. A launcher alone is insufficient if copied JS retains direct storage writers; the capability must terminate at a mutation broker or equivalent external authority. The repository has no native source/build, prebuilt platform matrix, signing/release mechanism, SEA flow, or private service installation.
- **Separate executable / SEA decision:** a true process-only executable can be a component only if its privileged implementation cannot be evaluated same-PID and mutation remains externally capability-gated. “Compiled” embedded JS and a shipped/static key do not qualify. Node SEA is not repository-supported, does not by itself establish the capability, and would require per-platform build/injection/signing plus Pi-layout work.
- **Permission decision:** a protected service/directory/key under a separate OS identity provides a real authority distinction, but requires administrator/service installation and changes the product from portable npm-only operation. This deployment requirement cannot be silently added to 0.2-E.
- **Authority inventory:** `dist/cli-entry.mjs` contains public pure validation/formatting; public read-only repository/runtime/human/Git observation; `PlanRevisionWriter` and plan/init/start mutation; process-bound trusted Pi loading, `runCli`, Pi/TUI lifecycle, `GuardianRunner`, extension factory/commands/handlers; and privileged GuardianStorage, ToolOperationTracker, SafePoint/latch/admission, HandoffService/artifacts, takeover, handoff, resume/recovery, authorization/dispatch, metrics and lifecycle writers. Moving that monolith behind another JS invocation is not a boundary.
- **Minimum secure architecture:** P0 public/attacker JS may transfer only untrusted command/target/terminal bytes to native launcher+mutation broker P1. P1 creates `CAP_priv` and a private inherited OS channel internally, retains authoritative storage mutation, derives a trusted runtime independently of parent JS state, and starts privileged Node P2. P2 owns GuardianRunner, genuine Pi 0.83.0, TUI and a narrow broker client. P2 sends versioned semantic requests bound to request/runtime/task/session/plan revision+digest/latch/handoff identities; P1 re-observes and enforces authorization/exactly-once state. No generic eval/module/function/Runner/storage writer API exists. P0 receives no endpoint, token, object, closure, factory, handler or writer.
- **Interactive/lifecycle implications:** Pi/TUI must live wholly in P2 and attach terminal input/output through P1; serializing handlers to P0 recreates the defect. EOF/client death can cancel only unadmitted work; admitted work uses existing durable outcomes/reconciliation. Human takeover/handoff/resume/recovery remain privileged TUI actions, with no 0.2-F durable stop/resume added. Windows requires explicit inherited-handle lists/restricted pipes and likely Job Object supervision; Linux/macOS require socketpair/fd and equivalent lifecycle work.
- **Feasibility result:** the capable architecture requires a native launcher/broker or protected OS service, a split of Runner/storage mutation, and Windows/Linux/macOS build/install/release support. This is not a bounded remediation compatible with the current universal npm package or 0.2-E scope. Detailed analysis, IPC/confused-deputy constraints, artifact classifications and process graph are recorded in `docs/0.2-e-r1-m-13-process-capability-spike.md`.
- **Bootstrap observation:** Node 22.19.0 reports `process.execPath` as writable and configurable; direct assignment succeeds. It cannot be the trust root. No full interception or post-fix copy/Worker/vm matrix was run because the mandatory architecture gate failed before a proof of concept.
- **Package/public observations:** `npm pack --dry-run --json` remains **12 files**, 193,454 packed bytes / 803,585 unpacked bytes; the new spike and reproducer are outside the `files` allowlist. Root `dist/index.mjs` still has **58** public string exports, null namespace prototype, and **0** enumerated privileged exports. These namespace facts do not close reconstructed bundle authority.
- **Tests actually run:** integrity/PR/diff inspection PASS; exact reconstruction reproducer **1/1 VULNERABLE as expected**; genuine Pi version proof **0.83.0**; installed-byte equality PASS; public export probe PASS; `process.execPath` mutability probe reproduced; npm dry-run inventory PASS. Reconstruction regression, bootstrap matrix, positive boundary, dependency attacks, plan-lock matrix, historical findings, UX regressions, reproducibility comparison and full suite were intentionally **NOT RUN** after the architecture stop condition.
- **Production/candidate state:** no production implementation was changed, no candidate was created, and no commit or push is authorized. PR #31 remains **OPEN / DRAFT**; no merge and no mark-ready. Independent gate remains **0/2**.
- **Final decision:** **R1-M-13 OPEN — R1-M-13 ARCHITECTURAL BLOCKER — 0.2-E REMEDIATION BLOCKED**. The minimum next decision is whether Aiopago may adopt a native capability launcher plus externally enforced mutation broker, or an administrator-installed protected service/permission boundary, including its platform packaging and deployment consequences. **DO NOT MERGE. DO NOT START 0.2-F.**

# CHECKPOINT — AIOPAGO 0.2-E R1-M-13 WORKER-BOUNDARY REMEDIATION

- **Previous candidate / integrity:** `a13d48feacebdde4a2e9e76f7321b70fb69ebb54` on `feat/unified-human-ux-0.2-e`; local/remote matched, merge-base was `6386d599e8510933786ea34822b01adac2fdb205`, tree was clean, and PR #31 was **OPEN / DRAFT**. Fresh Independent Review Round 1 was **BLOCKED** by `R1-M-13-REOPENED`; independent gate remains **0/2**.
- **Exact pre-fix Worker reproduction:** a genuine packed install and genuine `@earendil-works/pi-coding-agent@0.83.0` used `Worker(fileURLToPackedDistCliEntry)` with `execArgv --import attacker-preload`. Parent PID **3984** equalled Worker PID **3984**, Worker `threadId` was **1**, and writable `DefaultResourceLoader.prototype` interception captured **1** factory, **4** commands, **10** handlers and Runner authority **1**. Captured handlers persisted `OP-FORGED-WORKER` as `TERMINAL / KNOWN_SUCCESS / READ_ONLY`; captured `/aio takeover` persisted `HUMAN_TAKEOVER`. This independently reproduced the reviewer's same-PID finding before code changes.
- **Root cause / design decision:** `src`/`dist/cli-entry.mjs` constructed `GuardianRunner` through `runCli()` whenever Node supplied `import.meta.main`. Worker entry has entry-module identity without OS-process identity, so the privilege path was `Worker entry → runCli → GuardianRunner.create → genuine Pi resource loader → privileged extension factory`. The real process edge already existed at the bin's `spawnSync(process.execPath, ...)`, but the operational artifact could skip it.
- **Architecture selected:** the source operational entry keeps one lexical `aiopagoOperationalEntrypoint()` call for source execution; the deterministic package build removes that sole call from shipped `dist/cli-entry.mjs`. Thus every shipped JS artifact is non-privileged under import, direct execution and Worker entry. Both canonical and deprecated bin artifacts remain import-inert and, when supported executables, unconditionally spawn a sanitized fresh Node process. Only that clean child reads the exact physical dormant bundle, supplies its physical URL for trusted dependency resolution, and invokes the lexical function in child-local evaluated source. GuardianRunner and genuine Pi are first constructed there. No argv/environment/global/Worker predicate selects privilege.
- **Worker / forgeability evidence:** the exact packed Worker reproducer now reports genuine Pi **0.83.0**, factory **0**, commands **0**, handlers **0**, Runner authority **0**, `OP-FORGED-WORKER` absent and `HUMAN_TAKEOVER` absent. The actual tarball's four JS artifacts (both bin artifacts, `dist/index.mjs`, `dist/cli-entry.mjs`) pass direct absolute/file-URL imports plus Worker path/file-URL entries with genuine-Pi preload and malicious environment; copy and junction/symlink, query URL, CJS/ESM and cache/namespace routes remain covered. CommonJS mutation plus `syncBuiltinESMExports` successfully forged observed `isMainThread=true`, `threadId=0` and `parentPort=null` inside a real Worker; those values are classified **FORGEABLE / NO SECURITY ROLE**. `import.meta.main` is only an import-side-effect fact, not a process oracle.
- **Positive process / Pi proof:** a real packed executable with hostile parent preload/resolution environment observed bootstrap PID **6232** and distinct privileged PID **32364** (`6232 != 32364`), executable `C:\\nvm4w\\nodejs\\node.exe`, exact physical packed `dist/cli-entry.mjs`, target cwd, and Aiopago-owned physical Pi root. Final rebuilt local interactive proof observed **30192 != 23280**. Child environment strips `NODE_OPTIONS`, `NODE_PATH` and `PI_CODING_AGENT_ROOT`; parent Pi/prototype/fs/process state is not inherited. Pi remains exact **0.83.0** from Aiopago's trusted physical dependency slot.
- **Dependency / public boundary:** fake `NODE_PATH`, fake `PI_CODING_AGENT_ROOT`, hostile cwd, target/nested/ancestor modules, fake/duplicate Pi and parent patches cannot select child authority. Redirected/missing/wrong-version trusted Pi or pi-ai slots fail closed. Root/absolute `dist/index.mjs` retains exactly **58** public names and **0** privileged names; `GuardianRunner`, privileged `runCli`/Pi loader/factory, storage/tracker/takeover/handoff writers and their object graphs remain unreachable across copy/duplicate/descriptor/prototype/Symbol traversal. Direct execution of the shipped operational artifact is dormant even in a newly created attacker process. Deliberately reconstructing activated source or creating a separate process with attacker-selected preloads remains explicit source/process compromise outside runtime self-attestation; the supported CLI never depends on inherited preload state.
- **Lock / historical matrix:** takeover-lock **35/35 PASS** preserves exact live owner as `PLAN_WRITE_LOCKED`; dead/unknown/malformed/PID-reused owners require `PLAN_LOCK_RECONCILIATION_REQUIRED`; no automatic unlink occurs. Before/after import `process.kill`, `_kill` Proxy, `Function.toString`, descriptors, fs/platform attacks preserve owner, lock bytes/device/inode and `TASK_PLAN.md`. R1-H-13, R1-M-07, R1-L-09, R1-M-01..08, R2-H-01/R2-M-01/R2-L-01 and M-01..M-08 remain closed under their exact historical semantics.
- **Package / reproducibility:** npm tarball remains exactly **12 files**, with no source/tests/scripts/maps/reproducer leakage. Committed/fresh/packed bundles are byte-identical. Final hashes: `dist/index.mjs` SHA-256 `733cbf3435132dd9ec76617499d21b1806158fde0d2b1ea989e19ad45c18c634`; dormant `dist/cli-entry.mjs` `c024abf40d187705a2d0defdab1f18c2da7e286a637658c51ca289264f823716`. `npm pack` and diff-check pass; external consumers and read purity pass.
- **CLI / bounded scope:** real child-backed `aio init/status/why/next/plan`, `plan --check`, version/help and the deprecated executable pass. Real Pi planning retains two-record authorization and stop-after-plan behavior; interactive Runner authority is in the fresh child. Takeover, resume, recovery and guided/manual handoff regressions pass. 0.2-F durable stop/resume and 0.2-G/H/I/J remain **NOT STARTED**; no automatic handoff/resume/task execution was added.
- **Actual tests:** Worker/package security **7/7 PASS** (including two new Worker-specific tests and twelve all-artifact/copy/link/query Worker routes); 0.2-B **453 total / 451 PASS / 2 platform skips**; 0.2-C **30/30**; 0.2-D **28/28**; takeover/lock **35/35**; Pi **152/152**; trusted **69/69**; unified **43/43**; core **31/31**; human **34/34**. Full suite **950 total / 948 PASS / 2 platform skips / 0 fail**. `npm run check` PASS (**61 modules**).
- **Candidate / gate:** the new candidate is the commit containing this section (its immutable SHA is verified after commit/push because a commit cannot contain its own hash). PR #31 remains **OPEN / DRAFT**; no merge or mark-ready. Developer remediation contributes **0** independent rounds, so gate remains **0/2**.
- **Exact next step:** **FRESH INDEPENDENT REVIEW ROUND 1 ON THE NEW EXACT PUSHED SHA**. **DO NOT MERGE. DO NOT START 0.2-F.**

# CHECKPOINT — AIOPAGO 0.2-E R1-M-13-REOPENED REMEDIATION / INDEPENDENT-STYLE RE-REVIEW

- **Failed candidate / integrity:** `3b59ec18d832fd8a385e3b23d33e7075bcaf1978` on `feat/unified-human-ux-0.2-e`; remote matched, merge-base remained `6386d599e8510933786ea34822b01adac2fdb205`, PR #31 was and remains **OPEN / DRAFT**, and the starting tree was clean.
- **Exact reproduction before remediation:** a real packed install loaded genuine Pi `0.83.0`, replaced writable `DefaultResourceLoader.prototype.loadExtensionFactories`, then absolute-imported `dist/cli-entry.mjs`. The consumer captured **1** factory, **4** commands and **10** handlers. Captured `tool_call` / `tool_execution_end` closures created durable `OP-FORGED` as `TERMINAL / KNOWN_SUCCESS / READ_ONLY`; takeover remained zero in the reproduced chain. This faithfully reproduced **R1-M-13-REOPENED**.
- **Root cause:** the empty operational namespace was not a capability oracle. `dist/cli-entry.mjs` executed `runCli()` on every evaluation, so a filesystem absolute import initialized `GuardianRunner` and genuine Pi inside the already-prepatched consumer process instead of the intended fresh operational child.
- **Architectural remediation:** both shipped bin bootstraps and `src`/`dist/cli-entry.mjs` are entry-module-only. Node's ESM loader supplies each module's local `import.meta.main`; argv, environment, globals, Symbols, descriptors and caller module state cannot make an imported module the process entry module. An absolute/deep import therefore performs no CLI/Runner/Pi initialization. The supported executable starts the operational bundle as the entry module of a separate sanitized Node child, where `import.meta.main` is true and parent Pi/prototype state is absent. No hidden second privileged JavaScript entry was added.
- **Adversarial closure:** the packed exact reproducer now reports genuine Pi `0.83.0`, factory **0**, commands **0**, handlers **0**, Runner-backed authority **0**, Pi lifecycle/resource/event prototype calls **0**, `OP-FORGED` absent and `HUMAN_TAKEOVER` absent. Actual tarball enumeration covers all four shipped JavaScript files (`bin/aio.mjs`, `bin/eio.mjs`, `dist/cli-entry.mjs`, `dist/index.mjs`), package-copy and symlink/junction paths, CJS/ESM behavior, cache identities, namespace own keys/Symbols/prototypes and deep absolute paths. Operational entries are inert; `dist/index.mjs` retains only the exact 58-name public API and zero privileged names.
- **Operational child / dependency boundary:** runtime proof observed bootstrap PID **9952** and separate operational PID **26264** executing the exact physical `dist/cli-entry.mjs`. Init reported Pi **0.83.0**, resolved from Aiopago's owned physical dependency slot. `NODE_OPTIONS` preload reached only the non-privileged bootstrap; the child stripped it plus `NODE_PATH` and `PI_CODING_AGENT_ROOT`. Fake target, nested and ancestor `node_modules`, hostile cwd, fake/duplicate Pi and parent `fs/promises`/process patches did not enter the child. Trusted Pi/pi-ai symlink or junction redirection, missing slot and wrong version failed closed.
- **Lock / historical review:** takeover-lock **35/35 PASS** re-attests exact live owner, dead/unknown owner reconciliation, `process.kill`, Proxy `_kill`, `Function.toString`, descriptor and fs/platform attacks, exact lock bytes/device/inode, live owner and unchanged `TASK_PLAN.md`. R1-M-07 remains **SAFETY PRESERVED / AVAILABILITY CONTRACT NARROWED** with no automatic destructive recovery. A test-only live-owner hold was raised from 650 ms to 3.5 s because a cold real Windows `Get-CimInstance` probe measured ~2.6 s; this removes a false dead-during-probe race and changes no production timeout or expected outcome. R1-H-13, R1-M-07, R1-L-09, R1-M-01..06/08, R2-H-01/R2-M-01/R2-L-01 and M-01..M-08 remain closed under their original documented definitions.
- **Package / reproducibility:** **12 files**, with no `src`, tests, scripts, maps or reproducer leakage. Committed/fresh/packed bundles are byte-identical: `dist/index.mjs` SHA-256 `e42f974f21265c09b7a5e7dfd1eba1b7f0f74e5652ac2f8daf445fd2696b8f5e`; `dist/cli-entry.mjs` `dfc09601133cb3266b42ddadec965c28277166c1421bf476008aefa391a7c0ad`. `npm pack` PASS; package artifact **5/5 PASS**.
- **Regression results:** 0.2-B **453 total / 451 PASS / 2 skips**; 0.2-C **30/30**; 0.2-D **28/28**; takeover/lock **35/35**; Pi **152/152**; trusted **69/69**; unified **43/43**; core **31/31**; human **34/34**. Full suite **948 total / 946 PASS / 2 platform skips / 0 fail**. The increase from 946 is exactly two new R1-M-13 packed-artifact tests. `npm run check` PASS (**61 modules**).
- **Gate:** **R1-M-13 CLOSED**. Independent-style re-review found no new HIGH or MEDIUM. 0.2-E may be closed after review of the exact pushed candidate; PR #31 must not be merged by this remediation session and 0.2-F remains **NOT STARTED** until closure.

**STOP operativo:** non fare merge e non iniziare 0.2-F.

# CHECKPOINT — AIOPAGO 0.2-E FRESH ROUND 1 AUTHORITY-BOUNDARY REMEDIATION

- **Previous candidate / round:** `9c5294d3d77a40516d96b8abb3989e43b4378997`; Fresh Independent Review Round 1 = **BLOCKED**, gate **0/2**. Open findings were R1-H-13 and R1-M-13. Historical findings remain required closed unless actually regressed.
- **R1-H-13 root cause / architectural closure:** arbitrary pre-import same-process JavaScript could replace `_kill` with a callable Proxy that passed native-code heuristics and forged `ESRCH`. Aiopago no longer attempts JavaScript intrinsic authentication and no `PlanRevisionWriter` caller automatically unlinks an existing lock from any process-liveness or process-identity result. Exact LIVE produces `PLAN_WRITE_LOCKED`; DEAD, UNKNOWN, PID reuse, malformed metadata, unavailable probes and crash-stale state produce `PLAN_LOCK_RECONCILIATION_REQUIRED` (or stricter integrity rejection), preserving lock bytes and identity. Historical `.recovery` markers are preserved and never completed from liveness evidence. There is no deletion retry, timer or force flag.
- **R1-M-07 contract:** **SAFETY PRESERVED / AVAILABILITY CONTRACT NARROWED**. Automatic dead-owner and crash-owner recovery is intentionally removed. The human must inspect exact lock/marker state, ensure no Aiopago owner is operating, and explicitly reconcile stale state.
- **R1-M-13 root cause / architectural closure:** same-process `fs/promises` checks could not authenticate a privileged loader in that process. The packed package root now has an exact enumerated namespace of provider-neutral `createPlanAdapter`/`plan` plus read/data/validation/error APIs and exposes zero `GuardianRunner`, Pi loader, `runCli`, extension factory, storage/tracker/SafePoint or handoff authority. Source test injection remains source-only and is stripped from dist. Absolute `dist/index.mjs` has the same narrow namespace; side-effect-only `dist/cli-entry.mjs` exports nothing.
- **Operational process / Pi boundary:** `bin/aio.mjs` and `bin/eio.mjs` are thin bootstraps that launch the exact shipped operational entry in a fresh `process.execPath` child. The child environment strips `NODE_OPTIONS`, `NODE_PATH` and `PI_CODING_AGENT_ROOT`. Runner/Pi authority belongs only to this trusted-from-start operational process. Target JS and ambient package resolution are not executable authority. Deliberate child preloads, in-memory Node replacement and modification of installed Aiopago/dist/dependency bytes are outside the boundary and are documented as process/installation compromise, not defeated by runtime self-attestation.
- **Dependency model:** `@earendil-works/pi-coding-agent` is a normal exact production dependency at `0.83.0`, not a consumer-satisfied peer. Pi resolves only from Aiopago's physical npm dependency slots; pinned pi-ai resolves only inside exact Pi's dependency tree. `NODE_PATH`, target/cwd `node_modules`, `PI_CODING_AGENT_ROOT` and independent sibling search are absent. Physical redirect checks remain trusted-process layout validation/defense in depth; redirected, missing and wrong-version trusted slots fail closed before privileged evaluation.
- **Adversarial evidence:** a real Windows owner plus before/after-import attacks on `process.kill`, Proxy `_kill`/bound `_kill`, `Function.toString`, descriptors, `getSystemErrorName`, `process.platform`, filesystem reads and process helpers executes zero competing callbacks and preserves owner/lock/plan/latch. Real SIGKILL/dead owner and crash seams preserve published locks for reconciliation; historical recovery marker plus replacement lock remain byte/identity exact. A real tarball consumer patches `fs/promises` before `import "aiopago"`; the exact root list contains no privileged construction API and fake Pi evaluation/factory/handler capture, `OP-FORGED` and `HUMAN_TAKEOVER` are zero. Embedding-parent builtin patches cannot enter the fresh child; inherited preload/resolver environment is removed. Genuine exact Pi CLI init/read commands pass; ambient fake Pi is ignored; junction, missing dependency and wrong version reject.
- **Packaging / validation:** deterministic package remains 12 owned files with two bundles and no src/tests/scripts/maps/test support. Committed/fresh/packed dist are byte-identical: `dist/index.mjs` SHA-256 `e42f974f21265c09b7a5e7dfd1eba1b7f0f74e5652ac2f8daf445fd2696b8f5e`; `dist/cli-entry.mjs` `e0fb98e8e1efcba9844fdd9882383669b003b290c331ad61557c6ab9c91fd3cc`. Takeover/lock **35/35 PASS**; package artifact/process boundary **3/3 PASS**; trusted+unified **112/112 PASS**; accepted 0.2-B **453 total / 451 PASS / 2 platform skips**; 0.2-C **30/30 PASS**; 0.2-D **28/28 PASS**; portable alpha **22/22 PASS**. Full suite **946 total / 944 PASS / 2 platform skips / 0 fail**. `npm run check` PASS (**61 modules**); `npm pack --dry-run` PASS (**12 files**); diff check PASS.
- **Candidate / PR:** the new candidate is the commit containing this section; immutable SHA is appended to draft PR #31 after normal push. Base/merge-base remains `6386d599e8510933786ea34822b01adac2fdb205`. PR #31 remains **OPEN / DRAFT**. Gate remains **0/2**.
- **Next:** Fresh Independent Review Round 1 on the exact pushed candidate. 0.2-F remains **NOT STARTED / BLOCKED**.
- `checkpoint_message`: “0.2-E separates public and operational authority and requires manual stale-lock reconciliation; gate 0/2”.

**STOP operativo:** non mark-ready, non fare merge e non iniziare 0.2-F.

# CHECKPOINT — AIOPAGO 0.2-E FRESH ROUND 1 AMBIENT-ORACLE/PI-RESOLUTION REMEDIATION

- **Previous candidate / round:** `178294392b61116ec596f1b7eae4d590f818a97c`; Fresh Independent Review Round 1 = **BLOCKED**, gate **0/2**. Open findings were R1-H-12, R1-M-12 and R1-L-10. R1-H-11/R1-M-11 and every earlier finding remain required closed.
- **Exact pre-production reproductions:** on real Windows, a real child held `aiopago.plan-write-lock/0.3.0`; selective same-process replacement of `process.kill` forged `ESRCH` only for that live PID. Both replacement before Aiopago import and replacement after import removed the exact live lock, admitted competing takeover, engaged durable `HUMAN_TAKEOVER` and left the original owner alive with overlapping authority. A real npm tarball installed without its peer then selected fake Pi 0.83.77 plus fake pi-ai from `NODE_PATH`; a second install selected adjacent junctions. Both fake runtimes recovered one privileged factory, registered four commands and ten handlers before aborting.
- **R1-H-12 CLOSED in remediation:** production positive-absence evidence no longer calls the writable ambient `process.kill` wrapper. Aiopago captures Node's genuine native `_kill` binding once, validates that it is native at module initialization, and consumes its numeric system result directly. Missing/replaced native startup binding is `UNKNOWN`, never absence. The existing PowerShell diagnostic, start identity, nonce, hard-link recovery, inode/byte checks and ABA protocol remain unchanged.
- **R1-H-12 evidence:** independent real-child competitors patch `process.kill` before and after importing `PlanRevisionWriter`; both receive `PLAN_WRITE_LOCKED`, execute zero coordinated callbacks, preserve exact lock bytes/device/inode with no recovery marker, leave the owner alive and latch RELEASED. Genuine dead-owner/SIGKILL recovery remains green. Takeover/lock suite is **35/35 PASS**.
- **R1-M-12 CLOSED in remediation:** privileged Pi selection uses no `createRequire`, package resolver, `NODE_PATH`, target cwd, executable-relative global fallback or environment override. It examines only npm's nested and adjacent dependency layouts derived from Aiopago's own physical installation. Pi and pi-ai roots plus package manifests and entry files must be physical regular paths whose `realpath` is exact; junction/symlink redirection fails before either module is evaluated. Both dependency paths are validated before Pi code can receive the extension factory. Explicit public non-privileged `loadPi({ root })` behavior remains available.
- **R1-M-12 evidence:** two real-tarball installs independently attack fake Pi 0.83.77/fake pi-ai through `NODE_PATH` and adjacent junctions. Outcomes are respectively `PI_UNAVAILABLE` and `PI_TRUSTED_INSTALLATION_REDIRECTED`; fake module evaluation/factory/command/handler capture is zero and no runtime SQLite exists. Normal packed creation with the physical genuine dependency, previous Proxy/options/runtime-DI attacks, absolute-import inventory and object-graph authority checks remain green. Packed artifact security is **2/2 PASS**.
- **R1-L-10 CLOSED:** privileged `PI_UNAVAILABLE` guidance now requests a physical Aiopago npm dependency and does not mention `PI_CODING_AGENT_ROOT`, which remains supported only by ordinary non-privileged loader calls.
- **Packaging / regression gates:** deterministic package remains 12 files with bundled lexical authority and no source/test helpers; repeated builds are byte-identical (`dist/index.mjs` SHA-256 `74388e86214a9600ed3082096fee57fc3037d66defcd114d993153cb588544f7`, `dist/cli-entry.mjs` `b1c0e276e1dfcfb069389679dcc261eae4ee386c9346a1b99153e3b78368ca3f`). `npm run check` PASS (**61 modules**); `npm pack --dry-run` PASS (**12 files**); takeover/lock **35/35**, packed artifact **2/2**, trusted boundary plus unified UX **112/112**, portable/start/calibration focused gate **46 PASS / 1 platform skip**, portable-alpha **22/22**. Full suite **945 total / 942 PASS / 3 platform skips / 0 fail**. Developer evidence does not count toward the independent gate.
- **Candidate / PR:** the new candidate is the commit containing this section; its immutable SHA is appended to draft PR #31 after normal push because a commit cannot contain its own SHA. Base/merge-base remains `6386d599e8510933786ea34822b01adac2fdb205`. PR #31 remains **OPEN / DRAFT**. Gate remains **0/2**.
- **Next:** Fresh Independent Review Round 1 on the exact pushed candidate. 0.2-F remains **NOT STARTED / BLOCKED**.
- `checkpoint_message`: “0.2-E native lock absence and physical Pi resolution reject ambient forgery; gate 0/2”.

**STOP operativo:** non mark-ready, non fare merge e non iniziare 0.2-F.

# CHECKPOINT — AIOPAGO 0.2-E FRESH ROUND 1 WINDOWS-ORACLE/PI-FACTORY REMEDIATION

- **Previous candidate / round:** `c32e1048af7c6af150b254ec514e1c65ce216cc6`; Fresh Independent Review Round 1 = **BLOCKED**, gate **0/2**. Open findings were R1-H-11 and R1-M-11. R1-L-09, R1-M-01..M-08, R2-H-01/R2-M-01/R2-L-01, M-01..M-08 and the complete historical chain remain required closed.
- **Exact pre-production reproductions:** on real Windows, a real child held `aiopago.plan-write-lock/0.3.0`. A compiled fake `powershell.exe` first on PATH emitted exact `AIOPAGO_PROCESS_DEAD_V1`/exit 3, then syntactically valid `AIOPAGO_PROCESS_LIVE_V1:1`. On the previous candidate each attack deleted the live lock, admitted one competing coordinated callback while the original child was still alive, and let that callback replace plan bytes. From a real npm tarball, a Proxy around genuine Pi captured `resourceLoaderOptions.extensionFactories[0].factory`; manual registration produced 4 commands and 10 handlers, forged `OP-FORGED` as `TERMINAL/KNOWN_SUCCESS` with `file:attacker-chosen.txt`, and invoked captured `/aio takeover`, durably engaging `HUMAN_TAKEOVER`. The raw arbitrary `FORGED_REVIEW_EVENT` writer was not exposed by this callback-only reproduction; post-remediation regression nevertheless requires it absent.
- **R1-H-11 CLOSED in remediation:** Windows stale-lock deletion authority is now only a fresh native `process.kill(pid, 0)` `ESRCH`. PowerShell is diagnostic/exact-LIVE evidence only. Its DEAD sentinel, not-found result, LIVE start mismatch, malformed response, generic exit, access failure, timeout, executable absence, PATH/PATHEXT shadow and cwd shadow are never cleanup authority. If the PID exists, mismatch is `UNKNOWN`, including conservative PID reuse. Exact fake LIVE may preserve the lock as LIVE but cannot create mutation authority. Current-process identity may still be diagnostic metadata, but no identity value—attacker-controlled or otherwise—authorizes cleanup.
- **R1-H-11 evidence:** one real live owner was attacked with fake DEAD, fake LIVE wrong ticks, fake LIVE exact ticks, malformed output, PATHEXT manipulation and working-directory shadow. Every case kept owner nonce/bytes/device/inode exact, recovery marker absent, callback/plan/latch mutation zero and owner live. Empty PATH, unavailable PowerShell, generic exit, access error and timeout remain `UNKNOWN`. A real terminated/SIGKILL child and native nonexistent PID recover without manual deletion. Recovery marker/inode/byte/replacement-lock ABA checks, schema 0.3.0 and normal post-recovery plan mutation remain unchanged. One monotonic takeover deadline remains the only acquisition budget; no post-timeout timer or latch/event mutation exists.
- **R1-M-11 CLOSED in remediation:** packed production `GuardianRunner.create()` rejects any `options.pi` with `RUNNER_PI_INJECTION_FORBIDDEN` before root loading, services, resource loader, factory publication or durable mutation. It resolves the trusted installed Pi dependency itself while ignoring caller-selected `PI_CODING_AGENT_ROOT` for this privileged path. Ordinary injection of SessionManager, SettingsManager, ModelRuntime/model, metrics, context advisor, Ledger, storage, artifacts and tools is likewise rejected before reading getter values. Source tests use `createRunnerForInternalTest()` in the existing source-test marked region; package build physically removes it and `runnerForInternalTest()`.
- **Runtime/callback audit:** production owns the session manager associated with trusted Pi. The remaining `observeGit` is classified SAFE READ CALLBACK and is wrapped so caller code receives `this=undefined`, zero arguments, and no Runner/factory/storage/tracker/service. Environment warning receives only text; remaining options are data/path/policy inputs. `createGuardianExtension`, trusted Runner facade/authority, ToolOperationTracker, GuardianStorage and internal factory builders are absent from root and absolute packed namespaces. This trusts the Aiopago-selected installed Pi dependency; malicious replacement of that trusted dependency/package is outside the claim.
- **R1-M-11 evidence:** real tarball plus genuine-Pi Proxy, fake Pi, accessor and Proxy option tricks all reject with captured factories/registered commands/registered handlers = 0 and no runtime SQLite created. Every runtime dependency getter remains unread. Normal packed Runner creation through Aiopago-selected Pi succeeds. SQLite proves `OP-FORGED`, `FORGED_REVIEW_EVENT`, HUMAN_TAKEOVER, handoff, authorization, admission and dispatch evidence absent and latch unchanged. The direct object-graph attack still finds no raw DatabaseSync, real storage, tracker, SafePoint, metrics, lifecycle callbacks or guarded mutation authority.
- **Packaging:** exactly 12 files; no `src`, tests, scripts, source maps or internal source. `createGuardianExtension` and both source-test Runner helpers are absent from root/dist exports and packed bytes. Committed/fresh/packed dist are byte-identical (`dist/index.mjs` SHA-256 `20078a87fb9242af696c26874e272f0497e55f6dd2f1b77b8485ef0522adf041`; `dist/cli-entry.mjs` `8ed028f768799170a98d3a83a9998274c4271c49071268c6a8f82599bd99dd11`). Absolute imports retain only accepted namespaces.
- **Regression evidence:** takeover/lock **32/32 PASS**; real packed artifact **1/1 PASS**; Pi E2E **152/152 PASS**; trusted boundary **69/69 PASS**; unified UX **43/43 PASS**; core **31/31 PASS**; human workflow **34/34 PASS**; portable real Pi handoff **1/1 PASS**. Accepted 0.2-B **453 total / 451 PASS / 2 skips**, 0.2-C **30/30 PASS**, 0.2-D **28 total / 27 PASS / 1 skip**. Full suite **941 total / 938 PASS / 3 platform skips / 0 fail**. `npm run check` PASS (**61 modules** plus brand/build guards); `npm pack --dry-run` PASS (**12 files**); working/tree diff checks PASS.
- **Developer adversarial review:** attacked all 40 requested matrix classes: genuine/fake/unavailable/malformed/timed-out Windows probes, PATH/PATHEXT/cwd, exact/wrong ticks, real dead/native ESRCH/PID reuse/ABA; tarball Pi Proxy/fake/getter, all runtime DI, source-test/absolute namespace, callback `this`/arguments, direct graph and duplicate bundled boundary; forged operation/outcome/takeover and absence of journal/handoff/recovery/resume/dispatch evidence; R1-M-01/02/03/06/07, R2, M-08/M-07/M-06 and read purity. No known HIGH/MEDIUM remains; developer review does not count toward the gate.
- **Candidate / PR:** the new candidate is the commit containing this section; its immutable SHA is appended to draft PR #31 after normal push because a commit cannot contain its own SHA. Base/merge-base remains `6386d599e8510933786ea34822b01adac2fdb205`. PR #31 remains **OPEN / DRAFT**. Gate remains **0/2**.
- **Next:** Fresh Independent Review Round 1 on the exact pushed candidate. 0.2-F remains **NOT STARTED / BLOCKED**.
- `checkpoint_message`: “0.2-E Windows cleanup requires native ESRCH and privileged Pi factory delivery is production-owned; gate 0/2”.

**STOP operativo:** non mark-ready, non fare merge e non iniziare 0.2-F.

# CHECKPOINT — AIOPAGO 0.2-E FRESH ROUND 1 PROBE/EVIDENCE-AUTHORITY REMEDIATION

- **Previous candidate / round:** `cf30a7da3a959dae33df023e94c3254aa26fcd0c`; Fresh Independent Review Round 1 = **BLOCKED**, gate **0/2**. New findings were R1-M-09, R1-M-10 and LOW R1-L-09. R1-M-01..M-08, R2-H-01/R2-M-01/R2-L-01, M-01..M-08 and the full historical chain remain required closed.
- **Reproduced root causes:** on real Windows, a live child held `plan-write.lock`; removing PowerShell from `PATH` made the competing probe throw `ENOENT`, which the old generic catch classified `DEAD`, so stale recovery removed the live lock and ran the competing callback. A real installed npm tarball produced a real Runner whose `storage.appendEvent/admitOperation/finishOperation` and `toolTracker.finish/unknown` were reachable; `OP-FORGED` became terminal `KNOWN_SUCCESS` with attacker effect evidence and `FORGED_REVIEW_EVENT` entered the journal. A real 9.8-second holder completed takeover after the documented 10-second boundary.
- **R1-M-09 CLOSED in remediation:** process observation is tri-state and only positive OS evidence can return `DEAD`. Windows first accepts native `ESRCH`, otherwise requires exact structured PowerShell LIVE/DEAD sentinels plus dedicated exit classification and exact creation ticks. Missing PowerShell, generic exit 1, localized stderr, access/policy/spawn failure, timeout, parser failure and malformed/empty output are `UNKNOWN`. Linux only treats `/proc/<pid>/stat` `ENOENT` as absence; malformed/permission/boot-ID failures are `UNKNOWN`. Other platforms use native absence and otherwise fail closed. Only the current process's successful exact LIVE identity is cached. `UNKNOWN` cannot enter hard-link stale cleanup.
- **R1-M-09 evidence:** real live Windows owner plus absent PowerShell returns `PLAN_LOCK_OWNER_UNVERIFIED`; callback/latch/notification remain zero, owner stays alive and lock bytes/device/inode remain exact. Structured failure matrix, true nonexistent PID, real SIGKILL/dead owner, PID reuse, recovery-marker ABA, live lock, crash seams and post-recovery normal write are covered. Schema `aiopago.plan-write-lock/0.3.0`, PID/start identity, nonce, fsync publication and exact marker/inode/byte checks are unchanged.
- **R1-L-09 CLOSED:** one monotonic deadline object is created once per takeover invocation. Every lock attempt, identity probe, retry and sleep recomputes remaining time; Windows probe timeout is capped by that budget. The contract is exactly a 10-second **canonical takeover-authority acquisition** bound. SafePoint drain may finish afterward only when authority was acquired within the bound. Timeout leaves no pending timer/retry and cannot later engage the latch or append takeover evidence. Real 9.0/9.8/9.95/>10-second holder boundaries are covered.
- **R1-M-10 CLOSED in remediation:** real mutable Runner fields moved to a module-lexical WeakMap. The public Runner exposes frozen detached Ledger, runtime, storage, context and Git read facades only. Real GuardianStorage, ToolOperationTracker, SafePoint, AdmissionGate, HandoffService, ArtifactStore, metrics and Pi runtime/callback graph are not public properties. Internal Pi handlers receive an unforgeable closure-backed facade; direct operational/lifecycle methods without it fail before mutation. Public storage omits generic transaction/journal/operation writers, calibration identity binding, artifact indexing, metric writers and every specialized lifecycle writer.
- **R1-M-10 evidence:** a real external `.tgz` install creates a real Runner, recursively traverses own names/symbols/prototypes, attacks direct storage/tracker and guarded lifecycle surfaces, then inspects SQLite read-only. `OP-FORGED` and `FORGED_REVIEW_EVENT` are absent; SafePoint operation evidence is unchanged. Root/absolute exports contain no authority helper, raw DatabaseSync or source-test Runner capability. Normal actual Pi lifecycle, tool outcomes, handoff/recovery/resume, CLI and read projections remain covered.
- **Packaging:** deterministic bundled architecture remains 12 files with no `src`, tests, scripts, source maps or physical internal modules. Source-test Runner access is removed before bundling; production security is the lexical WeakMap/read-facade architecture, not an exported token or caller actor. Committed/rebuilt/packed `dist` is byte-identical.
- **Regression evidence:** takeover/lock **31/31 PASS**; real packed authority boundary **1/1 PASS**; Pi E2E **152/152 PASS**; trusted boundary **69/69 PASS**; unified UX **43/43 PASS**; core **31/31 PASS**; human workflow **34/34 PASS**; portable real Pi handoff **1/1 PASS**. Accepted 0.2-B **453 total / 451 PASS / 2 skips**, 0.2-C **30/30 PASS**, 0.2-D **28 total / 27 PASS / 1 skip**. Full suite **940 total / 937 PASS / 3 platform skips / 0 fail**. `npm run check` PASS (**61 modules** plus brand/build guards); `npm pack --dry-run` PASS (**12 files**); build/packed byte comparison and diff checks PASS.
- **Developer adversarial review:** reproduced all three findings first; attacked live-normal/PowerShell-absent/generic-exit/access/timeout/malformed/dead/PID-reuse/ABA probes, 9.0/9.8/9.95/>10-second takeover and post-timeout delay, real packed Runner own names/symbols/prototypes, forged journal/KNOWN_SUCCESS/tracker/calibration/artifact/transaction/metric/lifecycle surfaces, named/absolute imports, read purity, R1-M-01..M-08, R2 and historical recovery sentinels. No known HIGH/MEDIUM remains; developer review does not count toward the gate.
- **Candidate / PR:** the new candidate is the commit containing this section; its immutable SHA is appended to draft PR #31 after normal push because a commit cannot contain its own SHA. Base/merge-base remains `6386d599e8510933786ea34822b01adac2fdb205`. PR #31 remains **OPEN / DRAFT**. Gate remains **0/2**.
- **Next:** Fresh Independent Review Round 1 on the exact pushed candidate. 0.2-F remains **NOT STARTED / BLOCKED**.
- `checkpoint_message`: “0.2-E Windows probe and packed evidence authority fail closed; gate 0/2”.

**STOP operativo:** non mark-ready, non fare merge e non iniziare 0.2-F.

# CHECKPOINT — AIOPAGO 0.2-E FRESH ROUND 1 TAKEOVER/LOCK/PACKAGE REMEDIATION

- **Previous candidate / round:** `4461b7a4efdc4d652235020d0b911b9bfa63ba3b`; Fresh Independent Review Round 1 = **BLOCKED**, gate **0/2**. Open findings were R1-M-06, R1-M-07 and R1-M-08. R1-M-01, R1-M-02 safety, R1-M-03, R1-M-04/05, R2-H-01/R2-M-01/R2-L-01, M-01..M-08 and the historical chain remain closed.
- **Root causes:** takeover treated an early P1 observation as semantic consent and leaked transient `PLAN_WRITE_LOCKED`/plan drift as a terminal command result; the cooperative lock had only PID metadata and no dead-owner recovery; the npm exports map hid names but still shipped raw authority modules, source-test lifecycle helpers and a raw SQLite accessor that absolute file URLs could import.
- **R1-M-06 CLOSED:** takeover is now current-Runner task control authority. One production command owns monotonic 10-second bounded retry with 20–250 ms sleep/backoff only for `PLAN_WRITE_LOCKED`; no busy spin, second prompt, provider/model/network work or stale initial P1 consent. The successful narrow capability acquires PlanRevisionWriter coordination, reads the validated current plan inside it, requires the current Runner task ID, claims `HUMAN_TAKEOVER` synchronously in SQLite, releases coordination and then drains SafePoint. Notification occurs only after final canonical latch revalidation. Independent-process owner-first proves P1→P1′ may win and the same command then canonicalizes; takeover-first leaves zero P1′; P2-first and four bounded P2 writes converge without a second command; cross-task movement is terminal `HUMAN_TAKEOVER_TASK_CHANGED` with latch unchanged.
- **R1-M-07 CLOSED:** lock schema `aiopago.plan-write-lock/0.3.0` is closed and binds PID, OS process-start identity, 256-bit owner nonce, canonical timestamp and exact plan/Guardian paths. Metadata is fsynced before atomic hard-link publication. Exact live owner identity is never stolen. Missing PID or same PID with a different process-start identity proves the recorded instance dead; unknown identity and malformed/partial/foreign metadata fail closed. Dead cleanup claims the exact inode through a fixed hard-link recovery marker, revalidates bytes/inode/death, removes only that lock and lets another process finish interrupted cleanup. Replacement-lock ABA is detected and the replacement remains byte-exact. Real SIGKILL followed by fresh takeover recovers; normal plan apply also proceeds afterward. PID reuse, unknown proof, empty/partial/unknown schema/bad PID/bad identity/wrong path/unexpected field/bad timestamp and cleanup-vs-new-lock races pass.
- **R1-M-08 CLOSED:** npm now builds deterministic reviewable esbuild ESM bundles and ships only `dist/index.mjs`, `dist/cli-entry.mjs`, bins, selected docs and metadata—12 files; raw `src`, internal modules, tests, scripts and source maps are absent. Root and bin point to dist. Authority registries/functions remain lexical. A build transform physically removes source-test raw DB/lifecycle instrumentation before bundling, and production resume durable-count inspection uses a narrow structured attestation instead of a DatabaseSync accessor. Real packed external install enumerates and absolute-imports every shipped JS file, inventories exports, attacks `storage`, handoff-plan, plan-store, task-operation and plan-semantics names, owner mutation and object graph, and obtains neither authority helpers nor raw DatabaseSync. No handoff/latch/binding/recovery/admission/dispatch authority is exposed by the artifact. This does not claim protection from an independently privileged filesystem/SQLite editor.
- **Preserved closures:** exact C1 `RESUME_READY` / fresh S2 R1-M-01 remains `TASK_OPERATION_CONFLICT` before plan/history mutation and C1 resumes once; takeover-first/owner-first R1-M-02 safety, actual shutdown and lifecycle ABA R1-M-03, specialized owner mutation and public storage stubs remain closed. R2 stale resume, recovered-child task ownership, crash guidance, M-08 semantic recovery, M-07 final R*, M-06 lifecycle and all earlier findings remain covered.
- **Regression evidence:** takeover/lock **23/23 PASS**; real tarball physical boundary **1/1 PASS**; Pi E2E **152/152 PASS**; trusted boundary **69/69 PASS**; unified UX **43/43 PASS**; core **31/31 PASS**; human workflow **34/34 PASS**. Accepted 0.2-B **453 total / 451 PASS / 2 skips**; 0.2-C **30/30 PASS**; 0.2-D **28 total / 27 PASS / 1 skip**. Full suite **932 total / 929 PASS / 3 platform skips / 0 fail**. `npm run check` PASS (**61 modules** plus deterministic package build and instrumentation leak guard); `npm pack --dry-run` PASS (**12 files**); diff checks PASS.
- **Developer adversarial review:** attacked one live holder/one command, owner-first, takeover-first, P2-first, repeated P2 churn, task-ID drift, real dead owner, live-owner preservation, PID reuse/unknown identity, malformed locks, cleanup/replacement ABA, post-dead normal plan write, real tarball root/subpath/absolute imports, every shipped JS namespace, raw DB/helper names and public object graph; reran R1-M-01, R1-M-02/03, R2-H-01/R2-M-01/R2-L-01, M-01..M-08 and historical suites. No known HIGH/MEDIUM remains; developer review does not count toward the gate.
- **Candidate / PR:** the new candidate is the commit containing this section; its immutable SHA is appended to draft PR #31 after normal push because a commit cannot contain its own SHA. Base/merge-base remains `6386d599e8510933786ea34822b01adac2fdb205`. PR #31 remains **OPEN / DRAFT**. Gate remains **0/2**.
- **Next:** Fresh Independent Review Round 1 on the exact pushed candidate. 0.2-F remains **NOT STARTED / BLOCKED**.
- `checkpoint_message`: “0.2-E takeover liveness, dead plan lock and physical tarball authority closed; gate 0/2”.
...
# CHECKPOINT — AIOPAGO 0.2-E R1-M-13 PRODUCTION OPERATION AUTHORITY MIGRATION

- **Starting integrity:** local HEAD, tracked upstream and fresh `git ls-remote` for `origin/feat/unified-human-ux-0.2-e` were all `7ef2d831caaa16c5cf6ce44edae0c671a7e20e6f`; merge-base remained `6386d599e8510933786ea34822b01adac2fdb205`; branch remained `feat/unified-human-ux-0.2-e`; PR #31 remains **OPEN / DRAFT**; independent gate remains **0/2**. Existing CHECKPOINT/design/reproducer/native/persistent/elevated PoC unstaged material was preserved without reset, clean, stash or checkout-over.
- **Exact contract inventory:** `docs/0.2-e-operation-authority.md` records every operation writer/reader. Real call chain is Pi `tool_call` -> `ToolOperationTracker.admit` -> allowlist profile + durable `ACTIVE`; Pi `tool_execution_end` -> exact terminal classification -> `TERMINAL`; explicit `unknown` -> `UNKNOWN`; `SafePointCoordinator` reads before/after drain to preserve admitted atomic tools and reject active/ambiguous/unevidenced mutation; Handoff checkpoint consumes only SafePoint's returned operations. Legacy `GuardianStorage` `operations` remains user-owned portable state and is now labelled schema 1.1 as never canonical in SECURE mode. Latch/takeover admission authority is explicitly not migrated.
- **Architecture selected:** Option B, LocalService P1S plus unique service SID launches protected Node P2 and reuses the production `ToolOperationTracker` state machine against an operation-only protected SQLite authority. Option A native state-machine duplication and Option C two-store authenticity ledger were rejected. `PortableOperationAuthority` and `ProtectedSqliteOperationAuthority` carry distinct internal security labels; `requireSecureOperationAuthority` rejects portable objects. Secure missing/inaccessible/version-invalid state never catches/falls back to project SQLite.
- **Production files:** added `src/{operation-authority,protected-operation-authority,operation-authority-worker}.mjs`; changed `src/safety.mjs` so Tracker writes and SafePoint reads the selected authority; changed `src/runner.mjs` to label ordinary npm execution explicitly PORTABLE; changed only the `GuardianStorage` operation authority metadata, not unrelated tables or methods; added protected worker package build and deterministic `dist/operation-authority-worker.mjs`. Windows P1S/provision/oracle source is under `service/windows-operation-authority`; no generated executable is committed.
- **Protocol/boundary:** `aiopago.operation-authority-protocol/1`; fixed `OPERATION_ADMIT_TOOL`, `OPERATION_FINISH_TOOL`, `OPERATION_MARK_UNKNOWN`, exact retry, `OPERATION_GET`, and `OPERATION_LIST_TASK` frames; bounded private inherited stdin/stdout; P1/P2 PID, capability, protocol, service name/SID and actual P2 token binding. No eval, arbitrary SQL/module/function/Runner/GuardianStorage dispatch or public named mutation pipe. Readable/copied JS and a forged worker role marker convey no service SID, private channel, protected key or canonical DB access.
- **Provisioning/layout:** stable production identity is `AiopagoOperationAuthority`, with `%ProgramData%\Aiopago\OperationAuthority\{bin,canonical,runtime,control}`. Physical tests used only `AiopagoOperationAuthorityTest-37116-1787918295808`, root `%ProgramData%\Aiopago\OperationAuthorityTests\37116-1787918295808`, and service SID `S-1-5-80-3540240993-2232102863-246610338-3415692241-3362949126`, SID type **UNRESTRICTED**. Empty root/DACL/owner were established first through numeric SID ACLs; a real medium P0 denied enumerate/create/ACL before any copy; protected bytes/config followed; service registration was last; only P1S generated identity/DB bytes. Canonical directory is SYSTEM-owned and inherits an OWNER RIGHTS deny for WRITE_DAC/WRITE_OWNER, preventing a shared-LocalService file owner from rewriting child ACLs while the unique service SID retains data access.
- **Protected binary/store:** tested broker/Node/worker SHA-256 were respectively `5c6273a7f1ecc5ccb82872bfa3558d48f4a8847fbc49ae55efec9cfcfd4d0f01`, `995a3fb3cefad590cd3f4b321532a4b9582fb9c6575320ed2e3e894caac3e362`, `633f654f876bbc39402a637f6b877c114a274bc41a5e1430df239b99e7699568`, bound in protected config and checked before P2. Canonical `operations.sqlite` used WAL, `synchronous=FULL`, integrity/schema checks, protected request digests, and protected main/`-wal`/`-shm` files. P0 and other LocalService access were denied; P1S/P2 read/write passed.
- **Positive/restart:** P1S/P2 PIDs were `16408/31296` before and `34128/6564` after full stop/start. Fingerprint remained `baaf7d17446a072214f905b20deea621e93943ad22071d295cd307d32adca0d9`. `OP-PRODUCTION-BEFORE-RESTART` remained exactly `TERMINAL / KNOWN_SUCCESS / READ_ONLY`, generation 11. Post-restart `OP-PRODUCTION-AFTER-RESTART` reached `TERMINAL / KNOWN_SUCCESS / LOCAL_ATOMIC_MUTATION`, generation 12, `file:src/exact.txt`.
- **Direct SQLite/projection attack:** real Explorer-token medium P0 inserted user-writable `OP-FORGED-BY-P0` as `TERMINAL / KNOWN_SUCCESS / READ_ONLY`, forged generation 999999. Secure `OPERATION_GET` returned absent. P0 canonical read/write/copy/rename, key read/write, broker overwrite/rename, bin create, root/canonical ACL, service binary path/account/SID/delete all denied. Valid projection, deletion, higher generation and exact old replay had zero canonical effect.
- **Activated-source regression:** real medium P0 reused `test/reproducers/r1-m-13-activated-source.mjs` with genuine Pi 0.83.0, captured 1 factory, 4 commands and 10 handlers, and invoked the operation-producing handlers. Portable/project `OP-FORGED` became `TERMINAL / KNOWN_SUCCESS / READ_ONLY`; protected canonical `OP-FORGED` remained absent. One `HUMAN_TAKEOVER` event still occurred in the unmigrated authority, proving R1-M-13 overall remains open.
- **Crash/idempotency:** actual protected SQLite seam was `BEGIN IMMEDIATE` + terminal `UPDATE` + P2/P1S exit before COMMIT. Restart recovered `OP-PRODUCTION-CRASH` as the prior valid `ACTIVE` row with no partial terminal data; legitimate protected reconciliation then recorded `TERMINAL / UNKNOWN`. Same admission/terminal identity and payload returned protected idempotent results; changed admission/terminal payload returned `OPERATION_REQUEST_CONFLICT`; UNKNOWN semantics remained exact.
- **Cross-service sentinel:** separate unrestricted `AiopagoOperationAuthoritySentinel-*` ran as shared LocalService S-1-5-19 with a different service SID and without the Aiopago SID. Canonical read, write, ACL change, key access and protected broker modification all returned access denied. Sentinel service/root were removed.
- **Fail-closed lifecycle:** missing service was observed as SCM 1060; stopped service exposed no secure mutation path; held/missing DB failed `PROTECTED_STATE_INCOMPLETE`; expected-SID mismatch failed `SERVICE_IDENTITY_MISMATCH`; protocol mismatch failed `PROTOCOL_VERSION_MISMATCH`; no legacy fallback occurred. Linux/systemd and macOS/launchd are design only, not physical passes.
- **Tests:** current final core+operation run **37/37 PASS**; affected metrics/core/operation run **47/47 PASS** (10 metrics cases); trusted handoff/storage **69/69 PASS**; takeover/lock security **35/35 PASS**; real Pi E2E **152/152 PASS**; portable real-tool handoff **1/1 PASS**; package artifact/security **7/7 PASS**. Distinct affected total: **311/311 PASS**. Physical Windows production-domain oracle PASS. The whole repository suite was not run because the narrow affected matrix is green and package/security suites are expensive.
- **Package:** `npm pack --dry-run --json` PASS: **13 files**, 200,965 packed bytes / 836,486 unpacked bytes in that run, adding only inert-by-default `dist/operation-authority-worker.mjs`; no `src`, test, PoC, service source, generated native binary, map, ProgramData output or private key. Packed all-JS direct/Worker/preload matrix remains green; Windows Node 22.19 native Worker teardown is isolated per route and every returned authority row remains exact. Secret-pattern scan found none.
- **Tooling finding:** `npm run check` is blocked before checking changed code by the preserved Defender-quarantined phantom `poc/r1-m-13-persistent-authority/service-setup-preflight.ps1` (`UNKNOWN` open error). No Defender bypass/deletion/staging was attempted. All directly affected runtime/package suites passed independently.
- **Cleanup:** operation and sentinel services deleted; post-cleanup queries 1060; protected test roots, public outputs, P1S/P2/sentinel processes, handles, pipes, identity key and generated binaries removed. No production-looking canonical root was created or deleted. A temporary ignored Dev-Drive compile output was removed; the pre-existing Defender phantom remains preserved and must not be staged.
- **Commit/Git:** no commit or push was made. The worktree still combines intentionally preserved prior foundation evidence with this production slice, and one Defender-inaccessible phantom is enumerated; selective staging/commit in this session risks omitting or accidentally capturing mixed prior material. PR #31 remains **OPEN / DRAFT**, gate **0/2**, no merge/mark-ready, no final 0.2-E candidate.
- **Status/next:** **OPERATION JOURNAL: MIGRATED / PASS. TOOL OPERATION TRACKER: MIGRATED / PASS. GUARDIANSTORAGE OPERATION DOMAIN: SECURE CANONICAL SPLIT / PASS; PORTABLE LEGACY RETAINED. R1-M-13: OPEN — remaining latch/takeover, handoff/reservation/authorization, admission/dispatch, lifecycle/bindings, recovery/resume, plan and artifact authorities.** Recommended exact next unit: inventory and migrate the tightly coupled **latch/takeover plus narrow admission read**; do not implement it in this session.

```text
PRODUCTION OPERATION AUTHORITY: PASS
R1-M-13: OPEN — REMAINING PRIVILEGED AUTHORITY DOMAINS
INDEPENDENT GATE: 0/2
NEXT: SELECT AND MIGRATE NEXT COUPLED AUTHORITY DOMAIN
DO NOT MERGE
DO NOT START 0.2-F
```

**Suggested next session:** `aiopago-r1-m-13-latch-takeover-authority`

**Minimum resume prompt:** Continue R1-M-13 on `feat/unified-human-ux-0.2-e`; first require local/remote HEAD `7ef2d831caaa16c5cf6ce44edae0c671a7e20e6f`, preserve all unstaged foundation/operation-authority evidence, and read `docs/0.2-e-operation-authority.md`. Inventory coupling for the next bounded domain, recommended latch/takeover plus its narrow admission read; do not migrate handoff/resume/recovery broadly, do not claim R1-M-13 closed, keep PR #31 draft and gate 0/2, do not merge, and do not start 0.2-F.

# CHECKPOINT — AIOPAGO R1-M-13 OPERATION-AUTHORITY FOUNDATION CONSOLIDATION

- **Starting integrity:** local HEAD, tracked upstream, fresh `git ls-remote`, and PR #31 head were `7ef2d831caaa16c5cf6ce44edae0c671a7e20e6f`; merge-base was `6386d599e8510933786ea34822b01adac2fdb205`; branch was `feat/unified-human-ux-0.2-e`; PR #31 remained **OPEN / DRAFT**; independent gate remained **0/2**. No reset, clean, stash, checkout-over, force push, merge, mark-ready, latch/takeover migration, or 0.2-F work occurred.
- **Defender phantom:** `poc/r1-m-13-persistent-authority/service-setup-preflight.ps1` remained an inaccessible directory entry: MSYS/Git enumerated 1,636 bytes, Node/Git open failed, PowerShell `Test-Path` was false, and the filesystem security filter returned Windows error 225 (`ERROR_VIRUS_INFECTED`). It was the obsolete initial P/Invoke draft already replaced by `windows-setup-preflight.mjs` and referenced by no current procedure. PowerShell normal deletion could not address it; a normal Windows `del` removed the quarantined directory entry without opening or recovering bytes. Post-delete MSYS `test`, `find`, Git status/`ls-files --others`, Node `lstat`, PowerShell `Test-Path`, and directory enumeration all proved absence. No Defender setting, exclusion, recovery, ignore rule, check change, or bypass was used.
- **Exact pre-staging inventory:** every modified/untracked path was classified below. All 42 remaining paths are intended foundation/operation-authority source, tests, docs, reproducible package output, PoC source, or bounded non-secret evidence. Generated native executables, service output, ProgramData state, private key bytes, source maps, temp JSON, tarballs, and non-ignored logs were absent. Existing `.guardian/*.log` files remain ignored local test logs and are not commit content.

| Path | Git | Category | Secret-sensitive | Commit | Reason |
|---|---|---|---:|---:|---|
| `CHECKPOINT.md` | tracked | docs/evidence | yes | yes | preserved architecture, physical oracle, production migration, hygiene and gates |
| `dist/cli-entry.mjs` | tracked | generated production | yes | yes | deterministic package bundle for changed Runner/storage graph |
| `dist/operation-authority-worker.mjs` | untracked | generated production | yes | yes | inert-by-default deterministic protected worker bundle |
| `scripts/build-package.mjs` | tracked | build | no | yes | builds protected worker artifact |
| `src/runner.mjs` | tracked | production | no | yes | labels ordinary execution PORTABLE and selects portable operation authority |
| `src/safety.mjs` | tracked | production | no | yes | Tracker/SafePoint use selected operation authority |
| `src/storage.mjs` | tracked | production | no | yes | labels legacy operation rows portable/non-canonical in SECURE mode |
| `src/operation-authority.mjs` | untracked | production | no | yes | portable/secure authority interface, validation, fail-closed label gate |
| `src/protected-operation-authority.mjs` | untracked | production | yes | yes | protected operation-only SQLite canonical state machine |
| `src/operation-authority-worker.mjs` | untracked | production | yes | yes | private bounded P1S/P2 protocol worker |
| `docs/0.2-e-operation-authority.md` | untracked | docs | yes | yes | exact operation contract, physical result and remaining scope |
| `docs/0.2-e-r1-m-13-persistent-authority.md` | untracked | docs | yes | yes | persistent distinct-identity restart/crash evidence |
| `docs/0.2-e-r1-m-13-process-capability-spike.md` | untracked | docs | yes | yes | process capability decision and migration map |
| `test/operation-authority.test.mjs` | untracked | test | no | yes | secure/portable, forgery, crash, idempotency and fail-closed regressions |
| `test/package-artifact-security.test.mjs` | tracked | test | yes | yes | exact 13-file and inert protected-worker package boundary |
| `test/reproducers/r1-m-13-activated-source.mjs` | untracked | test/reproducer | no | yes | historical activated-source exploit and unmigrated takeover evidence |
| `test/reproducers/r1-m-13-operation-authority-p0.mjs` | untracked | test/reproducer | no | yes | real medium-P0 operation forgery probe |
| `test/reproducers/r1-m-13-provision-negative-probe.mjs` | untracked | test/reproducer | no | yes | pre-install protected-root negative probe |
| `service/windows-operation-authority/MediumTokenLauncher.cs` | untracked | production service source | yes | yes | Explorer-derived medium-token physical attack launcher |
| `service/windows-operation-authority/OperationAuthorityService.cs` | untracked | production service source | yes | yes | LocalService + unique-service-SID P1S broker host |
| `service/windows-operation-authority/ServiceConfigProbe.cs` | untracked | production service source | no | yes | scoped SCM/config negative probe |
| `service/windows-operation-authority/cleanup-test-service.ps1` | untracked | test/service | no | yes | exact scoped physical cleanup |
| `service/windows-operation-authority/provision-test-service.ps1` | untracked | test/service | yes | yes | owner/DACL-first scoped provisioning |
| `service/windows-operation-authority/set-sentinel-acl.ps1` | untracked | test/service | no | yes | cross-LocalService ACL sentinel setup |
| `service/windows-operation-authority/windows-physical-oracle.mjs` | untracked | test/service | yes | yes | real P0/P1S/P2 restart/crash/forgery oracle |
| `poc/r1-m-13-native-capability/README.md` | untracked | PoC docs | yes | yes | bounded process-capability result |
| `poc/r1-m-13-native-capability/broker.go` | untracked | PoC source | yes | yes | native ephemeral capability broker source; no binary |
| `poc/r1-m-13-native-capability/p2-runtime.mjs` | untracked | PoC source | yes | yes | bounded private-channel P2 source |
| `poc/r1-m-13-native-capability/poc.test.mjs` | untracked | PoC test | yes | yes | capability attack matrix; temp outputs only |
| `poc/r1-m-13-persistent-authority/README.md` | untracked | PoC docs | yes | yes | persistent authority procedures and evidence |
| `poc/r1-m-13-persistent-authority/cleanup-service.ps1` | untracked | PoC service | no | yes | exact PoC cleanup source |
| `poc/r1-m-13-persistent-authority/go.mod` | untracked | PoC build | no | yes | deterministic Go module metadata |
| `poc/r1-m-13-persistent-authority/go.sum` | untracked | PoC build | no | yes | dependency checksum metadata |
| `poc/r1-m-13-persistent-authority/medium-launcher/main.go` | untracked | PoC source | yes | yes | real Explorer-token P0 launcher source |
| `poc/r1-m-13-persistent-authority/p0-medium-attack.mjs` | untracked | PoC test | yes | yes | protected-state attack matrix source |
| `poc/r1-m-13-persistent-authority/p2-service-runtime.mjs` | untracked | PoC source | yes | yes | fixed semantic private P2 runtime |
| `poc/r1-m-13-persistent-authority/provision-service.ps1` | untracked | PoC service | yes | yes | distinct-identity protected provisioning source |
| `poc/r1-m-13-persistent-authority/same-user-anchor.ps1` | untracked | PoC test | yes | yes | NTFS/DPAPI/Credential/CNG rejection probes |
| `poc/r1-m-13-persistent-authority/same-user-anchor.test.mjs` | untracked | PoC test | yes | yes | same-user anchor harness |
| `poc/r1-m-13-persistent-authority/service-broker.go` | untracked | PoC source | yes | yes | restart-authentic service broker source; no binary |
| `poc/r1-m-13-persistent-authority/windows-elevated-oracle-evidence.json` | untracked | PoC evidence | yes | yes | bounded public fingerprints/SIDs/digests and PASS facts; no canonical/key bytes |
| `poc/r1-m-13-persistent-authority/windows-setup-preflight.mjs` | untracked | PoC test | no | yes | safe replacement elevated setup preflight |

- **Secret/evidence audit:** strict UTF-8 scan covered all 42 intended files. No private-key block, private key literal, capability literal, credential/token literal, environment dump, secret-like changed filename, generated native binary, map, protected SQLite/identity file, service output, or retained tarball was found. ProgramData strings occur only as documented target paths/procedures and bounded non-secret oracle evidence. Public service SIDs, fingerprints, hashes, request digests, test names and PIDs are evidence, not credential material. Result: **PASS**.
- **Quality correction:** the first post-phantom `npm run check` legitimately reached tracked/untracked source and found one deprecated compatibility-bin spelling in the new capability document. The document was made generic; no checker, allowlist, ignore rule, source root, filesystem error handling, or test was weakened. Final `npm run check`: brand guard PASS, deterministic three-entry build PASS, source-test instrumentation leak guard PASS, Node syntax **69 modules PASS**. Additional changed PoC/service syntax: 15 MJS PASS, 6 PowerShell files PASS, evidence JSON PASS. Go/.NET SDKs were no longer present on PATH; their already-recorded exact physical build/oracle evidence was not replaced by a claim.
- **Targeted regression:** core + operation authority **37/37 PASS**; metrics **10/10 PASS**; trusted handoff/storage **69/69 PASS**; takeover/lock **35/35 PASS**; genuine Pi **152/152 PASS**; portable real-tool handoff **1/1 PASS**; package artifact/security **7/7 PASS**. Distinct targeted total remains **311/311 PASS**. `git diff --check` PASS. No operation/ACL/protocol implementation changed during hygiene, so the already-recorded physical Windows production-domain oracle was not needlessly repeated.
- **Package:** actual `npm pack --json` PASS: **13 files**, 200,965 packed bytes, 836,486 unpacked bytes; the tarball was enumerated then deleted. It contains only README, two bins, three dist bundles, six allowlisted docs, and package metadata. No `src`, tests, scripts, PoC, service source, native executable/library, ProgramData output, map, key, secret, or source-test support leaked. `dist/operation-authority-worker.mjs` is the sole added package file and remained inert in the 7/7 security matrix.
- **Authority status:** operation journal / `ToolOperationTracker` operation domain is **MIGRATED / PASS**. `ProtectedSqliteOperationAuthority` is secure canonical authority under protected P1S/P2; `PortableOperationAuthority` retains legacy `GuardianStorage` operation rows only for PORTABLE compatibility. SECURE mode has no portable fallback and fails closed for missing/stopped service, protected-state failure, identity mismatch, or protocol mismatch. Direct project SQLite forgery remains non-canonical; restart, pre-COMMIT crash rollback, and idempotency contracts remain PASS.
- **Scope/gate:** this is one intermediate foundation checkpoint, not a complete 0.2-E candidate and not an independent-review candidate. R1-M-13 remains **OPEN** for latch/takeover; handoff reservation/authorization; admission/dispatch; lifecycle bindings; resume/recovery; plan authority; and artifact authority. PR #31 must remain **OPEN / DRAFT** and gate **0/2**. Exact next domain is **LATCH / TAKEOVER AUTHORITY MIGRATION**; it was not started here. Final commit SHA, normal push result and resulting remote head are necessarily post-commit facts and are recorded in the required session report rather than self-referentially inside this commit.

```text
OPERATION AUTHORITY FOUNDATION CHECKPOINT: READY TO COMMIT
PRODUCTION OPERATION AUTHORITY: PASS
R1-M-13: OPEN — REMAINING PRIVILEGED AUTHORITY DOMAINS
INDEPENDENT GATE: 0/2
NEXT: LATCH / TAKEOVER AUTHORITY MIGRATION
DO NOT MERGE
DO NOT START 0.2-F
```

# CHECKPOINT — AIOPAGO R1-M-13 LATCH / HUMAN_TAKEOVER AUTHORITY MIGRATION

- **Starting integrity:** local HEAD, tracked remote, fresh fetch, and PR #31 head were all `476090694a0b0edecc8c7b36035ce069a7e33104`; merge-base remained `6386d599e8510933786ea34822b01adac2fdb205`; branch remained `feat/unified-human-ux-0.2-e`; worktree was clean; PR #31 remained **OPEN / DRAFT** and independent gate **0/2**. No merge, mark-ready, force push, handoff reservation migration, broad authorization/admission/dispatch migration, resume/recovery, lifecycle, plan/artifact authority, or 0.2-F work occurred.
- **Recovered latch contract:** `docs/0.2-e-latch-takeover-authority.md` records the complete call-site inventory. Durable state is task-keyed `RELEASED|ENGAGED`, monotonic generation, nullable reason, engaged/released timestamps and actors, and exact last event. Bootstrap is `RELEASED/0/null`; released→engaged increments generation; ordinary engaged→`HUMAN_TAKEOVER` escalates reason at the same generation; duplicate same reason is idempotent; stale state/generation/reason is rejected; ordinary requests cannot downgrade takeover. SafePoint rereads exact state after every asynchronous wait. No generic release exists; historical ordinary release remains inseparable from trusted resume admission and was not migrated. `HUMAN_TAKEOVER` cannot be released by that route.
- **Architecture:** added explicit `PortableLatchAuthority` and secure latch label/gate. Portable `GuardianStorage` latch rows are compatibility/dev only and are labelled schema 1.1 non-canonical in SECURE mode. Secure latch authority extends the existing protected `operations.sqlite`; this sharing is required only because protected operation admission must compare exact canonical released generation and insert the operation in one `BEGIN IMMEDIATE` transaction. SECURE mode has no portable fallback. Secure handoff remains unavailable rather than crossing protected latch state into a user-owned reservation transaction.
- **Canonical schema/protocol:** protected schema upgraded only from `aiopago.operation-authority/1.0.0` to `1.1.0`; protocol upgraded from `/1` to `/2`. New tables are `latches` and append-only `latch_events`; the existing protected `authority_requests` ledger records payload digest/result/idempotency. Bootstrap/claim/escalation event, latch row and request result commit together. Current-schema missing latch tables fail closed. Main/WAL/SHM/temp siblings retain the existing protected canonical-directory ACL.
- **Production changes:** added `src/latch-authority.mjs`; extended protected SQLite authority and private worker; selected latch authority in `AdmissionGate`, `ToolOperationTracker`, `SafePointCoordinator`, Runner live status facade and portable Runner wiring; changed only the operation admission read/transaction needed for exact latch arbitration. P1S now bounds worker ready/request/complete waits and protocol 2. Fixed private operations include ensure/get/assert, ordinary SafePoint claim, and HUMAN takeover claim. There is no public named-pipe `TAKEOVER`, generic release, eval, arbitrary SQL/module/function/Runner/storage dispatch, or portable fallback.
- **SafePoint/admission:** secure SafePoint reads protected latch before drain, after abort, after `waitForIdle`, after `waitForNoStreams`, and at final arbitration. Takeover at each tested seam wins. Tool admission's final read and operation insert share one protected SQLite transaction. Twelve simultaneous cross-process races produced only the two valid linearizations: takeover first → no operation; admission first → exact admitted operation then takeover. No mixed stale-generation admission occurred.
- **Legitimate takeover:** scoped private P2 request `REQ-LEGITIMATE-TAKEOVER`, task `TASK-PRODUCTION-SECURE`, source `human:/aio-takeover`, P1S/P2 `37448/26708`, changed exact canonical `RELEASED/0/null` to `ENGAGED/1/HUMAN_TAKEOVER`. Canonical timestamps/actor/event were present and exact. This used the intended service-SID protected P1S/P2 channel, not a test sentinel or public RPC.
- **Priority/generation/ABA:** physical and unit matrices passed clear→ordinary, clear→takeover, ordinary→takeover, takeover→ordinary refusal, takeover→generic release refusal, duplicate takeover, changed-payload request conflict, and stale expected generation. Escalation retains generation 1 exactly. Existing status/consent tests reject same-visible-state later-generation ABA. The only production state cycle includes ordinary release inside unmigrated resume admission; no counterfeit standalone release was invented for this slice.
- **Direct P0 forgery:** real Explorer-derived medium P0 wrote plausible project `ENGAGED/HUMAN_TAKEOVER/999999` with reason/timestamps/actor/event identity; protected canonical remained `RELEASED/0` and secure admission remained open. After real canonical takeover, P0 wrote project `RELEASED/2147483647`; protected canonical remained exact takeover generation 1 and secure admission refused. Project delete, higher generation, replay, and forged operation had no protected effect. Canonical/key/broker/root/service mutation attempts were denied.
- **Activated-source regression:** genuine Pi `0.83.0`; factory 1; commands 4; handlers 10; Runner authority 1; portable `OP-FORGED` present; portable HUMAN takeover event 1; installed bytes unchanged. Protected `OP-FORGED` remained absent and protected takeover count did not change. A separate legitimate protected P2 takeover remained present. Result for latch domain: **PASS**; R1-M-13 overall remains open.
- **Restart/crash:** protected fingerprint `120ae0a73b8fdf60d18810a8afdadec1821aba2d8369f5d6e05ab31a08fd5120` remained stable. P1S/P2 changed `37448/26708 → 37364/29440`; P0 forged project clear while stopped; restart returned exact canonical `ENGAGED/1/HUMAN_TAKEOVER` and blocked secure admission. Latch crash seam was `BEGIN IMMEDIATE → latch/event/request update → P2/P1S exit before COMMIT`; restart returned exact prior `RELEASED/0`, then legitimate reconciliation produced `ENGAGED/1/HUMAN_TAKEOVER`. No partial generation/reason/event/request state was accepted.
- **Operation-domain regression:** protected operation before/after restart remained exact; direct project `OP-FORGED-BY-P0` and activated `OP-FORGED` remained canonically absent; pre-COMMIT terminal crash recovered exact ACTIVE then reconciled UNKNOWN; exact retries were idempotent; changed payloads conflicted; no fallback. Operation authority remains **MIGRATED / PASS**.
- **Cross-service/physical boundary:** final scoped service `AiopagoOperationAuthorityTest-33652-1787934312114`, unique service SID `S-1-5-80-1311746214-1847197804-2145715796-2788443630-1553489821`. A second unrestricted LocalService without that SID was denied latch read/write/generation mutation, operation authority, key, ACL and broker access. Explorer-derived P0 was medium integrity; P1S was LocalService plus unique service SID; P2 was its protected worker. Broker/worker hashes were `d5f3ede26b078cab11723f359cf815743d9c61c34f4d3ca887243e569caca366` / `7c934c42925736fc50e98c6810965f8a078f9cb6d0147843f99c7e8bef2b18d5`.
- **Fail closed:** missing service 1060, stopped service, missing protected DB, missing latch schema, identity mismatch, protocol mismatch, private-channel timeout, stale generation, and request conflict all failed closed. Missing schema stopped startup (`AUTHORITY_READY_TIMEOUT` after P2 rejected the schema); no schema was recreated and no project storage was consulted.
- **Tests:** final targeted non-package matrix **345/345 PASS** across core, latch/operation authority, takeover/lock, trusted handoff races, unified UX/status, genuine Pi E2E and portable real-tool handoff. Final package artifact/security **7/7 PASS**. Distinct final targeted total **352/352 PASS**. Windows physical operation+latch oracle PASS. `npm run check` PASS: brand guard and **71 modules** syntax. `git diff --check` PASS.
- **Package:** actual `npm pack --json` PASS: **13 files**, 204,396 packed bytes, 857,518 unpacked bytes. Final `dist/cli-entry.mjs` SHA-256 `0dc3102ebf52997a0214f61f53b2a97d2776ebb1a1e5b061a48491726299476c`; protected worker `7c934c42925736fc50e98c6810965f8a078f9cb6d0147843f99c7e8bef2b18d5`. No source, test, PoC, service source, native/C# binary, map, ProgramData output, key, secret, or test support entered the tarball. The worker remained inert under direct/Worker/preload attacks.
- **Cleanup/security scan:** scoped authority and sentinel services removed; SCM queries returned 1060; protected roots/public outputs/generated binaries/P1S/P2/sentinel processes/handles/pipes/identity removed; no key retained. Tarball removed. Intended paths contain no private key, capability, credential/token literal, generated native binary, SQLite state, map, or temporary service output.
- **Findings:** no new HIGH, MEDIUM, or LOW. Informational: secure handoff is intentionally fail-closed until protected handoff reservation can share exact latch arbitration; portable handoff keeps historical compatibility semantics only.
- **Commit/push:** one coherent intermediate commit is authorized after explicit-path staging and final diff/secret inspection. Its immutable SHA, normal push result, remote head and clean-tree proof are necessarily post-commit facts and are reported in the final session report rather than self-referentially in this commit.
- **Remaining domains / next:** operation and latch/takeover authority are migrated. R1-M-13 remains open for handoff reservation/authorization, broader admission/dispatch, lifecycle/bindings, resume/recovery, plan, and artifact authority. Exact next domain: **HANDOFF RESERVATION + AUTHORIZATION AUTHORITY**, because preserving takeover-first versus reservation-first ordering requires one protected transaction. Do not implement it in this slice.

```text
LATCH / TAKEOVER AUTHORITY: PASS
PRODUCTION OPERATION AUTHORITY: PASS
R1-M-13: OPEN — REMAINING PRIVILEGED AUTHORITY DOMAINS
INDEPENDENT GATE: 0/2
...
**Suggested next session:** `aiopago-r1-m-13-handoff-reservation-authorization`

**Minimum resume prompt:** Continue Aiopago 0.2-E R1-M-13 on `feat/unified-human-ux-0.2-e`. First verify local/remote HEAD equals the exact latch-authority commit reported by the prior session, worktree is clean, merge-base remains `6386d599e8510933786ea34822b01adac2fdb205`, PR #31 remains OPEN/DRAFT and gate 0/2. Read `docs/it/00-regole-operative.md`, the latest `CHECKPOINT.md`, `docs/0.2-e-operation-authority.md`, and `docs/0.2-e-latch-takeover-authority.md`. Inventory and migrate only the next coupled **handoff reservation + authorization authority** while preserving protected latch/reservation ordering; do not broaden to resume/recovery, lifecycle, plan/artifact authority or 0.2-F, do not merge or mark ready, and stop if the bounded transaction cannot be isolated.

# CHECKPOINT — AIOPAGO R1-M-13 HANDOFF RESERVATION / ACTIVE-SOURCE AUTHORITY MIGRATION

- **Starting integrity:** exact local/tracked/fresh remote HEAD `6f80e6be248ae19445b40f37fca4a826e3d28a21`; branch `feat/unified-human-ux-0.2-e`; merge-base `6386d599e8510933786ea34822b01adac2fdb205`; clean starting tree; PR #31 OPEN/DRAFT; independent gate 0/2. No merge, mark-ready, force push, 0.2-F, broad lifecycle, plan/artifact, recovery, resume authorization, admission or dispatch migration occurred.
- **Recovered handoff inventory:** `docs/0.2-e-handoff-reservation-authority.md` records writer/reader/fields/transaction/security role for project/protected handoff rows, active sources, reservation journals, request ledger, latch, invocation-local consent, P2 lifecycle epoch, bindings, artifacts, resume authorization, admission, dispatch and owner-gate plan authority. Historical stable identity is the exact 21-field reservation list already used by GuardianStorage, plus the complete initial projection and exact latch reason in the protected request digest.
- **Authorization taxonomy:** guided/manual decision and current-source verifier remain invocation-local; the protected reservation/active-source/event is the only migrated durable authority; no separate durable preparation-authorization object exists. Initial `NOT_AUTHORIZED / NOT_COMMITTED / NOT_STARTED` are empty-state reservation facts, not persisted YES. Project `authorizations` is exclusively resume authorization; project `admissions` and `dispatch_attempts` remain separate. Secure resume/recovery return `SECURE_RESUME_AUTHORITY_UNAVAILABLE` / `SECURE_RECOVERY_AUTHORITY_UNAVAILABLE` and never use portable fallback.
- **Selected schema/protocol:** existing protected `operations.sqlite` extends from 1.1.0 to `aiopago.operation-authority/1.2.0`; fixed private protocol extends to `/3`. Added `handoff_reservations`, protected `active_sources`, `handoff_reservation_events`; shared `authority_requests` remains the exact request ledger. Fixed operations are only `HANDOFF_RESERVE`, `HANDOFF_GET`, `HANDOFF_LATEST_TASK`, `ACTIVE_SOURCE_GET`. No general RPC/SQL/storage/Runner/resume/recovery method exists.
- **Transaction boundary:** one protected `BEGIN IMMEDIATE` validates exact protected latch state/generation/reason and no takeover, expected latest protected reservation, handoff/request identity and active source, then inserts reservation, active source, reservation event and request result. Projection occurs only afterward and cannot redefine canonical commit. Exact retry returns the recorded result; changed request/ID/provenance conflicts. Different task/same source and same source/different plan conflict; different source/same task is deliberately fail-closed until lifecycle transfer authority exists.
- **Direct P0 forgery / false negative:** Explorer-derived medium P0 PID 10012 wrote `HO-FORGED-BY-P0`, plausible `RESUMED`, active source, event, high latch/provenance and deleted real project `HO-PRODUCTION-HANDOFF`, replacing its project active source with `HO-FALSE-NEGATIVE-FAKE`. Canonical forged handoff/active source remained absent; real protected reservation digest `sha256:2016f1f597553a0da7ce13e4028c4a3ef032b6e4b326eefcd7946545f947b4df` and active source remained exact; conflicting secure reservation rejected.
- **Activated-source:** genuine Pi 0.83.0, factory 1, commands 4, handlers 10, Runner authority captured, handoff invocation attempted, portable operation forged and portable takeover recorded. Portable handoff rows happened to remain 0 at the reached boundary. Protected operation, takeover, reservation and active-source deltas from attacker P0 were all zero; domain result PASS.
- **Legitimate handoff:** genuine Pi 0.83.0 semantic integration used the real Runner/HandoffService/SafePoint path and protected implementation: task `TASK-E2E`; source `01a04e4d-4b1c-7d91-9ee5-2b4bf12986d9`; target `01a04e4d-52c4-70a6-8f0f-3d87917f149e`; Runner `RUNNER-25f2895e-af77-4ef6-add0-bfbd807d4edb`; plan `PLAN-E2E-1` / `sha256:01dd510c36d46b42967d1a715aa1fb985526f761f3f57fac00152a6ba232ac7a`; latch `ENGAGED/1/INTEGRITY`; reservation `HO-8db557e3122e61a75b1f3cf6`; checkpoint `CP-7e76944d0620d61302a5386f`; manifest `RM-ce48dca0a55f2cd447d32779`; active source exact; endpoint `RESUME_READY`; resume authorizations/admissions/dispatches 0. One forged portable authorization remained inert. The OS oracle separately exercised `HO-PRODUCTION-HANDOFF` through real protected service-SID P2.
- **Takeover/reservation:** historical before-entry, waitForIdle, waitForNoStreams and post-SafePoint tests remain green. Protected takeover-first produces zero reservation rows; reservation-first remains exact and a fresh post-reservation takeover blocks replacement while preserving durable arbitration evidence. Twelve independent protected races produced only reservation-first or takeover-first SQLite serializations.
- **Plan/owner/lifecycle boundary:** all historical revision-only, digest-only, plan/SafePoint, final coordination, owner CAS/rollback and current-source shutdown/same-ID ABA tests remain green. No global lock spans SafePoint. P2-local exact object/session/Runner/ACTIVE epoch attestation is sufficient immediately before this bounded transaction; durable lifecycle is not claimed. Secure subsequent lifecycle transfer/recovery is fail-closed, exposing lifecycle/session binding as the next domain.
- **Idempotency/rollback:** exact protected request retry is idempotent; changed same request is `HANDOFF_REQUEST_CONFLICT`; changed handoff provenance, source or task fails. Forced insert failures at each of `handoff_reservations`, `active_sources` and `handoff_reservation_events` leave handoff/active/event/request counts 0. Forced compatibility-projection failure leaves the already committed canonical reservation exact.
- **Restart/crash:** physical P1S/P2 changed `23876/32896 → 1100/14016` with fingerprint `3c50a2d1c564ebf5bf1650825cafb750fc3e27fe77d766c87134c4211409ce60`; real reservation/active source survived project deletion/forgery and restart. Crash seam after protected handoff+active inserts and before event/COMMIT recovered handoff/active/event/request absent. A committed reservation survived restart without relying on project projection.
- **Operation/latch regressions:** protected OP-FORGED remained absent; fake project takeover/clear had zero canonical effect; real takeover, admission/takeover concurrency, restart, crash, idempotency/conflict and no fallback passed. Operation and latch/takeover remain MIGRATED/PASS.
- **Cross-service / fail closed:** final scoped service `AiopagoOperationAuthorityTest-35692-1788020261070`, SID `S-1-5-80-1448388664-1666825740-2375972702-769161137-551297329`. Second LocalService lacked this SID and was denied handoff read/write, active-source/event mutation, latch, operation, key, ACL and broker access. Missing/stopped service, missing DB, missing latch or handoff schema, identity mismatch, protocol mismatch, worker-hash mismatch, channel/request timeout, stale generation, request conflict, stale latest identity, task/source conflict all failed closed with no project fallback.
- **Tests:** distinct targeted total **401/401 PASS**: protected handoff authority 11/11; operation+latch 14/14; trusted handoff/plan/owner/lifecycle 72/72; core+human+unified 108/108; genuine Pi E2E 153/153; takeover/lock 35/35; portable real Pi handoff 1/1; package security 7/7. Activated-source reproducer PASS for this domain. Elevated Windows physical oracle PASS. `npm run check` PASS with brand guard and 73 modules.
- **Package:** actual `npm pack --json` PASS, 13 files, 210,370 packed / 891,251 unpacked bytes. Only the existing inert protected worker changed in the package boundary; no source, service/C# source or binary, test, PoC, map, ProgramData state, SQLite, identity/key, test support or secret leaked. Package security 7/7 PASS; tarball removed; `git diff --check` PASS.
- **Cleanup:** authority and sentinel SCM queries returned 1060; scoped ProgramData roots removed; P1S/P2/sentinel/worker processes and handles absent; temporary outputs and tarball removed; no identity/key retained. Ignored `.guardian` logs contain bounded public test evidence only and are not staged.
- **Findings:** HIGH none; MEDIUM none; LOW none. INFORMATIONAL: project lifecycle/bindings, resume authorization/admission/dispatch, recovery, plan and artifact bytes remain non-canonical domains; secure status selects the protected reservation; a second task reservation remains unavailable until lifecycle transfer is protected.
- **Commit/push:** one coherent intermediate commit is authorized after final explicit-path staging. Its immutable SHA, normal push, remote-head and clean-tree facts are reported after commit because a commit cannot contain its own SHA. PR remains DRAFT and gate remains 0/2.
- **Remaining domains / next:** operation, latch/takeover and handoff reservation/active-source are migrated. R1-M-13 remains OPEN for lifecycle/session bindings; resume authorization/admission/dispatch; recovery; plan authority; artifact authority and other unrelated GuardianStorage domains. Exact next recommendation: **LIFECYCLE / SESSION BINDING AUTHORITY**. Do not implement it in this slice.

```text
HANDOFF RESERVATION AUTHORITY: PASS
LATCH / TAKEOVER AUTHORITY: PASS
PRODUCTION OPERATION AUTHORITY: PASS
R1-M-13: OPEN — REMAINING PRIVILEGED AUTHORITY DOMAINS
INDEPENDENT GATE: 0/2
...
**Suggested next session:** `aiopago-r1-m-13-lifecycle-session-binding-authority`

**Minimum resume prompt:** Continue Aiopago 0.2-E R1-M-13 on `feat/unified-human-ux-0.2-e`. First verify local/remote HEAD equals the exact handoff-reservation commit reported by the prior session, worktree is clean, merge-base remains `6386d599e8510933786ea34822b01adac2fdb205`, PR #31 remains OPEN/DRAFT and gate 0/2. Read operational rules, latest `CHECKPOINT.md`, and all three authority documents. Inventory and migrate only the next recommended **lifecycle / session binding authority** if it is independently bounded; preserve protected latch/reservation/operation transactions, keep resume authorization/admission/dispatch, recovery, plan/artifact authority and 0.2-F out of scope, do not merge or mark ready, and stop rather than widening if lifecycle cannot be isolated.

# CHECKPOINT — AIOPAGO R1-M-13 LIFECYCLE / SESSION BINDING AUTHORITY MIGRATION

- **Starting integrity:** exact local/tracked/fresh remote HEAD `d96ae947088b2fc60c4995a4700b70d2fbf52fe5`; branch `feat/unified-human-ux-0.2-e`; merge-base `6386d599e8510933786ea34822b01adac2fdb205`; clean starting tree; PR #31 OPEN/DRAFT; independent gate 0/2. No merge, mark-ready, force push, 0.2-F, resume authorization/admission/dispatch, general recovery, plan or artifact authority migration occurred.
- **Taxonomy/inventory:** invocation-local Runner lifecycle remains exact session object + session ID + Runner ID + monotonic in-memory epoch + ACTIVE, driven by genuine Pi `session_start`/`session_shutdown` identity through `ctx.sessionManager.getSessionId()`. Durable lifecycle is the reservation-bound replacement session/Runner/random binding nonce/incarnation/status/event relation. `docs/0.2-e-lifecycle-session-binding-authority.md` records writer/reader/field/transaction/security inventory. Project `runner_session_bindings` and binding journal rows are now explicitly portable compatibility projections, never secure authority.
- **Recovered state machine:** repository states are exactly `ACTIVE` and terminal `SUPERSEDED`. Protected create requires an existing canonical reservation and exact reservation Runner/binding nonce; the committed identity is handoff, replacement session, Runner, random binding ID and current lifecycle incarnation. Only exact `ACTIVE → SUPERSEDED` is legal. Exact retries are idempotent; stale incarnation/session/Runner/binding and conflicting reason/payload reject; no revive/delete/LWW path exists.
- **Schema/protocol:** same protected `operations.sqlite` upgraded one way from schema 1.2.0 to `aiopago.operation-authority/1.3.0`; protocol upgraded to `aiopago.operation-authority-protocol/4`. Added `lifecycle_bindings` and append-only `lifecycle_binding_events`; shared `authority_requests` remains request digest/result ledger. Fixed private operations are create/get/get-by-session/transition/events only. No arbitrary bind/SQL/storage/Runner/resume/recovery API was added.
- **Production choreography:** external Pi paused-target creation remains outside SQLite. After runtime custom-binding and exact ACTIVE lifecycle attestation, protected binding row/event/request commit atomically; only then is project binding projection rewritten from canonical proof. Protected commit failure keeps the target paused, reservation exact and canonical binding absent with bounded project failure evidence. Canonical commit followed by projection failure preserves canonical success and fails closed. Secure continuity/status reads canonical binding; project forgery cannot decide them.
- **Genuine Pi binding:** real Pi 0.83.0 task `TASK-E2E`, source `01a04f61-390e-75ef-9ecd-3a7a69208e29`, target `01a04f61-3e6e-7a0b-a53c-2d26b1f4f650`, Runner `RUNNER-f604c00b-6a55-4ab0-9945-ae791b7ed051`, handoff `HO-4ffd1c4e9badf036a8cd0de0`, binding `BIND-8ce3cf04-e27a-4f34-b2e8-a88c3075694b`, incarnation 3, status ACTIVE, checkpoint `CP-295bab49d983d6d7ba7dc497`, manifest `RM-cfab6d31be9f8a64a28c19c9`; endpoint `RESUME_READY`; resume authorization/admission/dispatch counts 0. A forged portable YES remained inert.
- **Pi lifecycle / ABA:** genuine session shutdown superseded the exact ACTIVE canonical target with reason `session_shutdown`; same textual ID start advanced to E2 but did not revive E1. Shutdown before final source reservation remains `HANDOFF_SOURCE_CHANGED`; shutdown during both SafePoint waits and same-ID ABA remain rejected; unrelated identified shutdown leaves S1 unchanged. Historical M-03, M-06 and M-07 matrices remain green.
- **Second transfer:** the previous fail-closed task-reservation conflict is narrowed exactly: latest protected reservation must have an ACTIVE binding whose target equals the new source and Runner equals the new reservation Runner. Genuine Pi completed protected `S1 → S2 → S3` reservation/binding transfer while both targets stayed paused and resume remained unavailable. Superseded/ABA source still rejects at the lifecycle boundary; secure recovery remains unavailable.
- **P0 project attacks:** real medium P0 inserted fake ACTIVE binding, fake Runner/binding/session and later/newer fake SUPERSEDED binding, deleted the real project handoff/binding and replaced its active source/binding. Canonical `HO-PRODUCTION-HANDOFF / SESSION-PRODUCTION-TARGET / RUNNER-PROTECTED-P2 / BIND-HO-PRODUCTION-HANDOFF / incarnation 1 / ACTIVE` remained exact across restart. Forged canonical binding and transition counts were zero.
- **Activated source:** genuine Pi 0.83.0, factory 1, commands 4, handlers 10, Runner authority captured, portable `OP-FORGED` and one portable HUMAN takeover; portable handoffs/active sources/bindings/binding events were 0 at the reached boundary. Protected operation/takeover/reservation/active-source/binding/transition deltas were all zero. Installed bytes remained exact.
- **Races/atomicity:** shutdown-first before reservation/binding rejects stale lifecycle; binding-first then shutdown yields exact ACTIVE then SUPERSEDED; synchronous final attestation + protected transaction admits only one serial order; same-ID E2 cannot reuse terminal E1. Forced protected binding insert failure after external target creation retained paused target plus exact reservation/no binding. Crash after lifecycle row+event mutation and before COMMIT recovered exact ACTIVE and one bind event, then explicit transition succeeded.
- **Physical Windows oracle:** final scoped service `AiopagoOperationAuthorityTest-29616-1788039936721`; SID `S-1-5-80-3370430299-2047767891-41658664-3674529862-16332396`; P1S/P2 `10776/34764 → 25536/11020`; fingerprint stable `c2c3a144ae1f9e7071fe8f7978f3c9b75475da0d98ea263244b211ddfaab9ba3`. Broker/Node/worker hashes were `fc1895f388a2254329d13cc1109ad7f26414da6673681f19a45aac2300251dbc`, `995a3fb3cefad590cd3f4b321532a4b9582fb9c6575320ed2e3e894caac3e362`, `67bf06bd35c3d01db6deb922e09aab30ef7a58c055d721f15e6227006e447fc7`. Real protected binding survived P0 projection attack/restart; exact transition, duplicate and conflict passed; lifecycle pre-COMMIT crash rolled back.
- **Cross-service/fail closed:** second unrestricted LocalService without Aiopago SID was denied canonical/binding read, binding write, lifecycle transition/event, handoff, latch, operation, key, ACL and broker access. Missing/stopped service, missing DB, missing latch/handoff/lifecycle schema, corrupt lifecycle schema, identity mismatch, worker hash mismatch, protocol mismatch, timeout/EOF, stale generation/binding/incarnation, request conflict, unexpected transition and forged project state all failed closed with no portable fallback.
- **Prior-domain regressions:** protected OP-FORGED absent; fake project latch/takeover had zero canonical effect; handoff reservation/active source remained exact; takeover/reservation and operation/takeover ordering, restart, crashes and no-fallback tests remained PASS. Operation, latch/takeover and handoff reservation remain MIGRATED/PASS.
- **Tests:** protected operation+latch+handoff+lifecycle authority **32/32 PASS**; trusted handoff/plan/owner/lifecycle **74/74 PASS** (72/72 full plus shutdown-first and protected-binding-failure oracles 2/2); core+human+unified **108/108 PASS**; genuine Pi E2E **153/153 PASS**; takeover/lock **35/35 PASS**; portable real Pi handoff **1/1 PASS**; package security distinct cases **7/7 PASS** (initial run hit the known Node 22.19 native Worker teardown after 6 passes; exact isolated route passed 1/1 and the bounded fresh-process retry is now symmetric). Distinct targeted total **410/410 PASS**. Activated-source and elevated Windows physical oracle PASS. `npm run check` PASS with brand guard and 75 modules; `git diff --check` PASS.
- **Package:** actual `npm pack --json` PASS: 13 files, 214,133 packed / 917,208 unpacked bytes. Only existing inert protected worker package boundary changed; no source, tests, PoC, C# service source/binary, maps, ProgramData state, SQLite, identity/key, test support or secrets entered the tarball. Tarball removed.
- **Cleanup/findings:** authority and sentinel SCM queries returned 1060; scoped ProgramData/public roots, generated binaries, P1S/P2/sentinel processes, handles/pipes, tarballs and identity material removed. HIGH none; MEDIUM none; LOW none. INFORMATIONAL: Node 22.19 Windows Worker teardown remains a known native flake handled only by fresh-process retry; resume/recovery/plan/artifact authority remains unavailable.
- **Commit/push:** one coherent intermediate commit is authorized after final explicit staging/inspection. Immutable SHA, normal push, remote head and clean tree are post-commit report facts. PR remains DRAFT; gate remains 0/2.
- **Remaining/next:** operation, latch/takeover, handoff reservation/active source and lifecycle/session binding are migrated. R1-M-13 remains OPEN for resume authorization + exactly-once admission/dispatch, recovery, plan authority, artifact authority and unrelated privileged storage domains. Exact next recommendation is **RESUME AUTHORIZATION + EXACTLY-ONCE ADMISSION/DISPATCH**, because protected lifecycle removes its prerequisite; do not implement it here.

```text
LIFECYCLE / SESSION BINDING AUTHORITY: PASS
HANDOFF RESERVATION AUTHORITY: PASS
LATCH / TAKEOVER AUTHORITY: PASS
PRODUCTION OPERATION AUTHORITY: PASS
R1-M-13: OPEN — REMAINING PRIVILEGED AUTHORITY DOMAINS
...
**Suggested next session:** `aiopago-r1-m-13-resume-admission-dispatch-authority`

**Minimum resume prompt:** Continue Aiopago 0.2-E R1-M-13 on `feat/unified-human-ux-0.2-e`. First verify local/remote HEAD equals the exact lifecycle-authority commit reported by the prior session, clean tree, merge-base `6386d599e8510933786ea34822b01adac2fdb205`, PR #31 OPEN/DRAFT and gate 0/2. Read operational rules, latest `CHECKPOINT.md`, and all four authority documents. Inventory and migrate only the next coupled **resume authorization + exactly-once admission/dispatch authority** if it is one independently bounded transaction/saga; preserve protected lifecycle/handoff/latch/operation authority, keep general recovery and plan/artifact authority plus 0.2-F out of scope, do not merge or mark ready, and stop rather than widening if the domain cannot be isolated.
