# CHECKPOINT — AIOPAGO 0.2-B PLAN PROPOSAL FOUNDATION COMPLETE

- **Baseline/main:** `28c04a426b9c78c81ef58fb1b719f24585b947f2` (`docs: close Aiopago rename migration`). **Branch/worktree:** `feat/plan-proposal-foundation-0.2-b`, `F:/dev/aiopago-0.2-b`. **HEAD applicativo:** `686ee629ac75ebff5659928d12417fe1ddfec1bf`; il commit documentale che contiene questa sezione chiude la slice senza iniziare 0.2-C.
- **Authority e Plan Port:** `TASK_PLAN.md` Markdown resta l'unica autorità del piano. `PlanRevisionWriter` è la sola boundary di replacement: lock cooperativo atomico, doppia attestation, temp esclusivo, write, file fsync, close, rename atomico, directory fsync best-effort e cleanup. Nessun piano canonico o sync bidirezionale è stato aggiunto a SQLite.
- **Proposal/validation:** `PlanProposal` immutabile `aiopago.plan-proposal/0.1.0` riceve soltanto proposta strutturata, base revision/digest esatti, nuova revision, requirements, timestamp, producer/reason e candidate plan. `validateTaskLedger()` è l'unica semantica estratta da `TaskLedger.validate()` e conserva required fields, status, path bounds, DAG/cicli, DONE evidence e lifecycle current/next.
- **CAS/concorrenza:** apply richiede contemporaneamente revisione e SHA-256 dei byte esatti; ogni mismatch è `PLAN_CAS_CONFLICT`, senza merge, last-write-wins o retry automatico. Tutti i writer Aiopago sono serializzati dallo stesso lock; lock stale/corrotto fallisce senza wait. La seconda attestation avviene subito prima del replace. Resta esplicitamente non garantita la race stretta con editor non cooperativo fra read finale e rename.
- **Materialization/diff:** materializzazione offline/provider-neutral aggiorna soltanto metadata revision/requirements/updated e blocco JSON deterministico, preservando prose e bytes esterni; LF e CRLF coperti. `aiopago.plan-diff/0.1.0` separa added/removed/changed per campi piano e `task_item_id`, con ordering stabile e nessun runtime/private reasoning.
- **Provenance/idempotenza:** `aiopago.plan-revision/0.1.0` e result `aiopago.plan-apply-result/0.1.0` sono concreti e testati; `previous_revision_id` e digest esatti sono preservati, nessuna approval è inventata. Record immutable `prepared.json`/`applied.json` sotto `.guardian/plan-proposals/` sono sola provenance derivata e recovery. Stesso proposal ID/payload restituisce lo stesso result anche dopo restart; payload diverso fallisce chiuso.
- **Legacy policy:** lettura pre-rename resta supportata e read-only; Plan Proposal mutation richiede migration esplicita e non riscrive silenziosamente. La compatibility owner-gate preesistente resta operativa come eccezione specializzata. Il `TASK_PLAN.md` root storico non è stato usato come target mutating e ha diff zero.
- **Regressioni:** `satisfyOwnerGate()` delega al writer comune mantenendo comando canonico, actor umano, HANDOFF_CONFIRM, BLOCKED→SATISFIED, nuova revisione, lifecycle/item/next-step, atomicità e idempotenza; E2E Pi owner-gate PASS. Le view 0.2-A restano read-only e `NEEDS_ATTENTION`/`RUNTIME_NOT_VERIFIED`; nessun Core Observation Port, Intent Adapter, provider/planning LLM, `aio start` o comando plan mutating è stato introdotto.
- **Test/gate:** test mirati Plan Proposal **28/28 PASS** su repository Git temporanei reali; `npm test` **184/184 PASS** (156 baseline + 28 nuovi); `npm run check` **PASS**, brand guard + 42 moduli; `npm pack --dry-run` **PASS, 33 file**; `git diff --check` **PASS**; `git diff -- TASK_PLAN.md` vuoto. Failure injection copre validation/temp-create/write/fsync/replace/post-commit recovery; CAS, due writer e compatibility coperti offline.
- **Review:** diff `origin/main...HEAD` ispezionato per lost update/TOCTOU, doppio writer/autorità, revision reuse, proposal collision, prose/EOL, legacy/owner-gate, temp/recovery e scope accidentale. Zero finding HIGH/MEDIUM noto.
- **Residual LOW/nonblocking:** nessuna primitive portabile disponibile rende atomico il compare-and-replace contro editor umani non cooperativi; directory fsync resta best-effort come nella writer precedente; lock stale richiede ispezione/rimozione umana esplicita. **Roadmap:** 0.2-B CLOSED. **Next:** 0.2-C — Intent Adapter minimo, in sessione separata.
- `checkpoint_message`: “Aiopago 0.2-B completa: Plan Proposal foundation CAS-safe, deterministica e senza intent adapter”.

**STOP operativo:** 0.2-B chiusa. Non iniziare 0.2-C nella stessa sessione.
