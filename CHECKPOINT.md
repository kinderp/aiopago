# CHECKPOINT — AIOPAGO 0.2-D ROUND 1 H-01/M-01 REMEDIATION

- **Stato/gate:** Independent Review Round 1 sul candidate precedente `418db3934d844bd701fc02a7dadd726e64c20b88` = **BLOCKED**. H-01 e M-01 sono rimediati; 0.2-D torna `READY FOR INDEPENDENT REVIEW`, gate nuovo `0/2`; developer review non conta. Issue #27; draft PR #28; branch `feat/start-objective-0.2-d`; worktree `F:/dev/aiopago-0.2-d`; `origin/main` e merge-base `488c4fd696003a6987ea50996bbab644e63ab56c`.
- **H-01/root cause:** `createStdinAuthorizer()` troncava al primo newline e interpretava EOF come submit, quindi `yes` senza terminatore e `yes\nno\n` autorizzavano. Il nuovo parser è una state machine byte-level bounded (1024 byte) con stati record/CR-attesa-LF/completo: approva soltanto un record esatto `y|yes` case-insensitive terminato LF/CRLF. EOF pre-terminatore, whitespace, controllo/binario, CR solo, oversize, stream error ed extra byte/record negano. Pipe/file viene consumato fino a EOF; TTY non attende EOF ma rifiuta extra byte già consegnati col record.
- **H-01/evidence:** matrice production authorizer e vero packed/unpacked `aio` copre y/yes/case, LF, CRLF split, `ye`+`s\n`, EOF vuoto e dopo y/yes, multiline in uno o più chunk, blank/whitespace/alternative, controllo/binario, oversize e stream exception. Ogni denial integrato attesta `plan.apply=0` e `TASK_PLAN.md` byte-esatto; preview→authorize→stessa frozen proposal→apply e una sola observation/planner call restano invariati.
- **M-01/root cause:** Pi 0.83.0 `createAgentSessionServices()` esegue `DefaultResourceLoader.reload()`→`SettingsManager.reload()` e cancellava gli override applicati prima dei servizi. Gli override vengono ora applicati dopo quel reload e prima di `createAgentSessionFromServices()` sul medesimo manager consultato dall'AgentSession: `retry.enabled=false`, retry agent `maxRetries=0`, provider `maxRetries=0`, `compaction.enabled=false`; getter runtime fail-closed attestati prima della sessione.
- **M-01/persistenza:** gli override sono invocation-local via `applyOverrides`, non marcano campi persistenti e non vengono salvati/flushed. Il precedente `flush()` finale è rimosso. Snapshot di project/global settings verifica esistenza, hash/byte, size e mtime invariati su success, retryable 429, malformed output, provider cancellation e user DENY; file assenti non vengono creati. `SessionManager.inMemory()` resta attivo e nessun JSONL/transcript viene scritto.
- **Real SDK evidence:** actual resolved `@earendil-works/pi-coding-agent 0.83.0`, settings global+project con retry/compaction true, resource reload reale e provider offline controllato. Prima risposta retryable `HTTP 429`, seconda risposta valida disponibile: **1 call esatta**, seconda non consumata, `START_PLANNER_UNAVAILABLE`. Scenario usage oltre context window: **1 call**, nessuna compaction. Success/malformed/cancel/DENY restano tutti a massimo una provider call senza rete.
- **Contratti preservati:** nessun production diff ai core 0.2-B/C. Observed-base A, stale durante planning, CAS dopo approval, owner gate, strict JSON/duplicate fields, no-tools/resource isolation, exact display/authorize/apply, no-auto-execution, target/CWD, `eio` e package boundary restano coperti e verdi.
- **Test:** Node `v22.19.0`; focused 0.2-D **23/23 PASS**; Intent Adapter **30/30 PASS**; Plan Proposal **453 total, 451 PASS, 2 platform skip, 0 fail**; full **662 total, 660 PASS, 2 platform skip, 0 fail**; `npm run check` PASS (brand guard + 49 moduli); `npm pack --dry-run` e tarball reale PASS (40 file). Install esterno/consumer PASS con packed CLI `yes\n`/`yes\r\n` apply e `yes` EOF/`yes\nno\n`/empty EOF deny, public `plan.*` e subpath interno bloccato.
- **Developer adversarial review:** AUTH attaccato con timing EOF, split chunk/CRLF, record doppio stesso/successivo chunk, trailing whitespace/data, controllo/binario, >1024 e stream error. PI attaccato con global/project precedence, reload post-override, 429, source compaction true, near-overflow, provider/malformed/cancel, settings presenti/assenti e transcript. GENERAL stale, owner gate, exact proposal, no-auto-execution, no-tools/resources e strict JSON rieseguiti. Nessun finding H/M noto; self-review non conta.
...
- `checkpoint_message`: “0.2-D Round 1 H-01/M-01 rimediati: auth record fail-closed e Pi settings post-reload non persistenti; gate 0/2”.
