# CHECKPOINT — AIOPAGO 0.2-B NEW INDEPENDENT REVIEW ROUND 1 REMEDIATION COMPLETE

- **Stato:** `0.2-B = READY FOR INDEPENDENT REVIEW`; gate indipendente `0/2`. Ogni nuovo HEAD azzera il gate; questa developer review non conta. 0.2-C resta `NOT STARTED / BLOCKED`.
- **Base/perimetro:** il feature HEAD iniziale `51271fc0b75a62571e988b471e9fbff5d24caf59` è stato sincronizzato con `origin/main=323782ee7c491dfbf097606b7313f4d7c98865b1` tramite merge normale `450032d`; nessuna riscrittura/force. `docs/efficiency-benchmark-requirement.md` è contenuto base di main, preservato byte-identico e non implementato.
- **H-01:** candidate, history e intent sono completi/durevoli; ownership lock viene attestata prima della FINAL authority observation. Dopo exact revision+byte CAS non avvengono lock read, callback o I/O: soltanto confronti in memoria e immediato rename. La regressione muta il piano durante lock attestation e la final observation rileva il conflitto.
- **H-02/H-03:** generic `PlanProposal` preserva semanticamente `owner_gate` o ne conserva l'assenza; soltanto `satisfyOwnerGate()` HUMAN può transizionarlo. `PlanPort` ricostruisce sempre i canonical own data fields e ricalcola il digest, senza fidarsi di `instanceof`, subclass, prototype o digest caller.
- **H-04/H-05:** il limite authority condiviso reader/writer di 32 MiB è verificato prima di temp/history/intent/rename; registration/diff/commit record hanno limiti separati. Witness inode/device non attribuisce l'attore: candidate senza durable live post-rename `applied.json` produce sempre `PLAN_RECOVERY_AMBIGUOUS`, inclusi external write/copy/orphan-temp rename e kill reale post-rename. Nessun recovered success viene inventato; result live affidabile resta idempotente.
- **M-01/M-02:** Task e TaskItem `DROPPED`/`SUPERSEDED` richiedono `terminal_reason`, `terminal_actor`, `terminal_at`; supersession TaskItem richiede relazione existing/non-self/reciproca/non-contraddittoria e aciclica. Tutta l'authority usa UTF-8 fatal; byte malformati falliscono chiuso, Unicode e BOM validi sono preservati.
- **L-01/L-02:** attempt 129 non viene creato; a 128 senza result si ottiene `PLAN_ATTEMPT_LIMIT_REACHED` con `ABANDON_PROPOSAL`, mentre result applicato resta ispezionabile. Directory open/fsync/close inattesi emergono; soltanto errori/platform esplicitamente unsupported sono best effort, distinguendo consistenza byte e durability power-loss.
- **Regressioni/scope:** vecchi H1-H4/M1-M5, nuovi H-01..H-05/M-01..M-02/L-01..L-02 e owner gate sono coperti. 0.2-A resta read-only; root `TASK_PLAN.md` è invariato; nessun Intent Adapter, 0.2-C o Efficiency Benchmark.
- **Gate:** Node `v22.19.0`; Plan Proposal **156 test: 155 PASS, 1 platform skip, 0 fail**; full suite **312 test: 311 PASS, 1 platform skip, 0 fail**; `npm run check` PASS; `npm pack --dry-run` PASS (33 file); diff/check e root authority PASS.
- **Residual LOW soltanto:** irreducibile transizione user-space→rename contro editor non cooperativo; directory sync esplicitamente unsupported non garantisce power-loss durability; ACL/Windows extended metadata non sono preservabili atomicamente con Node standard; stale lock e recovery ambiguous richiedono riconciliazione umana; amministratore same-account fuori threat model.
- `checkpoint_message`: “0.2-B new Round 1 remediation completa; pronta per independent review, gate 0/2”.
