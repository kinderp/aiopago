...
Inside the Runner, `/aio` defaults to the same shared human status projection used by the CLI. `/aio status`, `/aio why`, `/aio next` and `/aio plan` are deterministic and read-only; `/aio status technical` and `/aio plan technical` disclose secondary diagnostics. Context-threshold consent now invokes the existing trusted handoff path directly instead of inserting a command, while replacement sessions remain paused until a separate explicit resume confirmation. See [`docs/0.2-e-unified-human-ux.md`](docs/0.2-e-unified-human-ux.md).
